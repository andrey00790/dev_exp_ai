...
def docker_services():
    ...
    subprocess.run(
        ["docker-compose", "-f", "tests/e2e/docker-compose.yml", "down", "-v"], cwd="."
    )
...
def jira_client():
    ...
    return Jira(url="http://localhost:8080", username="admin", password="admin")


@pytest.fixture
def confluence_client():
    """Клиент Confluence для тестов"""
    return Confluence(url="http://localhost:8090", username="admin", password="admin")
...
class TestConfluenceIntegration:
    ...
    def test_confluence_pages_search_multilingual(
        self, confluence_client, load_test_data
    ):
    ...
    def test_confluence_page_content_russian(self, confluence_client, load_test_data):
        ...
        page = confluence_client.get_page_by_title(
            space="TESTSPACE", title="Руководство по API Gateway"
        )
...
class TestElasticsearchIntegration:
    ...
    def test_documents_indexed(self, elasticsearch_client, load_test_data):
        ...
        en_query = {"query": {"match": {"content": "OAuth authentication"}}}

        en_results = elasticsearch_client.search(index="test_documents", body=en_query)
        assert en_results["hits"]["total"]["value"] > 0

        # Поиск по русскому запросу
        ru_query = {"query": {"match": {"content": "API Gateway реализация"}}}

        ru_results = elasticsearch_client.search(index="test_documents", body=ru_query)
    ...
    def test_multilingual_search(self, elasticsearch_client, load_test_data):
        ...
        results = elasticsearch_client.search(index="test_documents", body=query)
...
class TestCrossSystemIntegration:
    ...
    def test_semantic_search_cross_system(
        self, jira_client, confluence_client, elasticsearch_client, load_test_data
    ):
        ...
        es_results = elasticsearch_client.search(index="test_documents", body=query)
    ...
    @pytest.mark.asyncio
    async def test_async_data_synchronization(
        self, jira_client, confluence_client, elasticsearch_client, load_test_data
    ):
        ...
        search_query = {"query": {"match": {"title": "E2E Integration"}}}

        results = elasticsearch_client.search(index="test_documents", body=search_query)
