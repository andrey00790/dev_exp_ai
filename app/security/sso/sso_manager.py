...
class SSOManager:
    ...
    async def initiate_login(self, provider_id: int, request: Request, db: Session) -> Dict[str, Any]:
    ...
    async def handle_logout(self, user_id: int, provider_id: Optional[int], db: Session) -> Optional[str]:
        """Handle SSO logout"""
        ...
        if provider_id:
            ...
            if provider.provider_type == SSOProviderType.SAML and provider.slo_url:
                ...
                return f"{provider.slo_url}?SAMLRequest=..."  # Proper SLO request needed
    ...
    def update_provider(
        ...
        if provider.provider_type == SSOProviderType.SAML:
            ...
            for field in ["client_id", "client_secret", "authorization_url", "token_url", "userinfo_url", "scope"]:
                if field in update_data:
                    setattr(provider, field, update_data[field])
    ...
    def delete_provider(self, db: Session, provider_id: int):
        ...
        active_users = db.query(SSOUser).filter(
            SSOUser.provider_id == provider_id,
            SSOUser.active == True
        ).count()
    ...
    def get_user_sso_info(self, db: Session, user_id: int) -> List[Dict[str, Any]]:
        ...
        sso_users = db.query(SSOUser).filter(
            SSOUser.user_id == user_id,
            SSOUser.active == True
        ).all()
    ...
    async def _initiate_saml_login(self, provider: SSOProvider, request: Request) -> Dict[str, Any]:
        """Initiate SAML login"""
    ...
    def _validate_provider_config(self, provider_data: Dict[str, Any]):
        ...
        required_fields = {
            ...
            SSOProviderType.OAUTH_CUSTOM: ["name", "client_id", "client_secret", "authorization_url", "token_url"]
        }
...
class SSOConfig:
    ...
    def from_env(cls) -> "SSOConfig":
        ...
        config.require_encryption = os.getenv("SSO_REQUIRE_ENCRYPTION", "true").lower() == "true"
