...
from models.document import (CommonFilters, Document, DocumentStatus,
...
class TestMetadataPerformance:
    ...
    def test_document_creation_performance(self):
        ...
        assert (
            creation_time < 2.0
        ), f"Создание 1000 документов заняло {creation_time:.2f}s"
        assert len(documents) == 1000

        print(
            f"Создание 1000 документов: {creation_time:.3f}s ({creation_time/1000*1000:.1f}ms на документ)"
        )
    ...
    def test_document_serialization_performance(self):
        ...
        ), f"Сериализация 100 документов заняла {serialization_time:.2f}s"
        ...
        print(
            f"Сериализация 100 сложных документов: {serialization_time:.3f}s ({serialization_time/100*1000:.1f}ms на документ)"
        )
    ...
    def test_filter_building_performance(self):
        ...
        for i in range(1000):
            filter_obj = (
                ...
                .by_date_range(
                    datetime.now() - timedelta(days=30), datetime.now(), "updated_at"
                )
            )
        ...
        assert (
            building_time < 1.0
        ), f"Построение 1000 фильтров заняло {building_time:.2f}s"
        ...
        print(
            f"Построение 1000 сложных фильтров: {building_time:.3f}s ({building_time/1000*1000:.1f}ms на фильтр)"
        )
    ...
    def test_common_filters_performance(self):
        ...
        assert (
            building_time < 0.5
        ), f"Построение 1000 предопределенных фильтров заняло {building_time:.2f}s"
        assert len(filters) == 1000

        print(
            f"Построение 1000 предопределенных фильтров: {building_time:.3f}s ({building_time/1000*1000:.1f}ms на фильтр)"
        )
    ...
    def test_document_factory_methods_performance(self):
        ...
        for i in range(100):
            # Confluence документ
            confluence_doc = Document.from_confluence_page(
                confluence_data, f"confluence_{i}"
            )
            ...
            # GitLab документ
            gitlab_doc = Document.from_gitlab_file(
                gitlab_file_data, gitlab_project_data, f"gitlab_{i}"
            )
        ...
        assert (
            factory_time < 1.0
        ), f"Создание 300 документов через фабричные методы заняло {factory_time:.2f}s"
        assert len(documents) == 300

        # Проверяем что документы созданы правильно
        confluence_docs = [
            d for d in documents if d.source_type == SourceType.CONFLUENCE
        ]
        ...
        print(
            f"Создание 300 документов через фабричные методы: {factory_time:.3f}s ({factory_time/300*1000:.1f}ms на документ)"
        )
    ...
    def test_large_metadata_handling(self):
        ...
        large_metadata = {
            ...
            "nested_structure": {
                f"level1_{i}": {
                    f"level2_{j}": {
                        f"level3_{k}": f"value_{i}_{j}_{k}" for k in range(10)
                    }
                    for j in range(10)
                }
                for i in range(10)
        ...
        assert (
            handling_time < 1.0
        ), f"Обработка больших метаданных заняла {handling_time:.2f}s"
    ...
    def test_filter_combination_performance(self):
        ...
        assert (
            combination_time < 0.5
        ), f"Комбинирование 100 фильтров заняло {combination_time:.2f}s"
        assert len(combined_filters) == 100

        print(
            f"Комбинирование 100 сложных фильтров: {combination_time:.3f}s ({combination_time/100*1000:.1f}ms на комбинацию)"
        )
...
class TestMemoryUsage:
    ...
    def test_document_memory_efficiency(self):
        ...
        assert (
            total_estimated_size < 1024 * 1024
        ), f"1000 документов занимают {total_estimated_size} байт"

        print(
            f"Приблизительный размер 1000 документов: {total_estimated_size/1024:.1f}KB ({estimated_size_per_doc}B на документ)"
        )
