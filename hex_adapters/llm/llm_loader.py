...
from .providers.base import LLMModel, LLMRequest, LLMResponse
...
class LLMConfig:
    ...
    # priority, cost_optimized, quality_optimized, balanced, round_robin
...
class EnhancedLLMClient:
    ...
    def __init__(self, config: LLMConfig):
        ...
        provider_count = len(self.router.providers)
        logger.info(f"Enhanced LLM Client initialized with {provider_count} "
                    "providers")
    ...
    def _setup_providers(self):
        ...
        if self.config.mode in ["local", "multi"]:
            ...
            try:
                ...
                logger.info(f"✅ Ollama provider added: "
        ...
        if self.config.mode in ["openai", "multi"]:
            ...
            if OPENAI_AVAILABLE and self.config.openai_api_key:
                try:
                    ...
                    logger.info(f"✅ OpenAI provider added: "
        ...
        if self.config.mode in ["anthropic", "multi"]:
            ...
            if ANTHROPIC_AVAILABLE and self.config.anthropic_api_key:
                try:
                    ...
                    logger.info(f"✅ Anthropic provider added: "
                                f"{self.config.anthropic_model}")
                except Exception as e:
        ...
        if not self.router.providers:
            logger.error("No LLM providers available! Falling back to basic "
                         "Ollama.")
            ...
            try:
                ...
                raise RuntimeError(f"Failed to initialize any LLM provider: "
                                   f"{e}")
    ...
    async def generate(
        ...
        **kwargs
        ...
        if self.config.cost_limit_per_request > 0:
            ...
            min_cost = (min(cost for _, cost in estimated_costs)
                        if estimated_costs else 0)
            if min_cost > self.config.cost_limit_per_request:
                logger.warning(
                    f"Request exceeds cost limit: ${min_cost:.4f} > "
    ...
    async def get_available_models(self) -> Dict[str, List[str]]:
        ...
        for provider_type, provider in self.router.providers.items():
            try:
                ...
                if (hasattr(provider, 'get_available_models') and
                        callable(getattr(provider, 'get_available_models'))):
                    provider_models = await provider.get_available_models()
                    models[provider_type.value] = provider_models.get(
                        "models", []
                    )
                else:
                    models[provider_type.value] = [provider.model.value]
            except Exception as e:
                logger.warning(f"Failed to get models for "
                               f"{provider_type.value}: {e}")
...
def load_llm_config() -> LLMConfig:
    ...
    return LLMConfig(
        ...
        enable_fallback=(os.getenv("LLM_ENABLE_FALLBACK", "true").lower() ==
                         "true"),
