...
from .models import SSOProvider, SSOUser, SSOSession, SSOProviderType
...
class SAMLAuthHandler:
    ...
    def init_saml_auth(self, request: Request, provider: SSOProvider) -> OneLogin_Saml2_Auth:
        ...
        saml_request = {
            ...
            'server_port': str(url_data.port) if url_data.port else ('443' if url_data.scheme == 'https' else '80'),
        ...
        if hasattr(request, '_body') and request._body:
            try:
                ...
                import urllib.parse
                saml_request['post_data'] = urllib.parse.parse_qs(request._body.decode())
    ...
    async def handle_acs_response(
        ...
        try:
            ...
            if errors:
                ...
                raise HTTPException(status_code=400, detail=f"SAML authentication failed: {'; '.join(errors)}")
    ...
    async def handle_sls_request(
        ...
        try:
            auth = self.init_saml_auth(request, provider)
            url = auth.process_slo(delete_session_cb=lambda: self._delete_saml_session(db, auth.get_nameid()))
    ...
    def _map_saml_attributes(self, attributes: Dict[str, Any], nameid: str) -> Dict[str, Any]:
        """Map SAML attributes to user data"""
        # Common SAML attribute mappings
        attr_mapping = {
            'email': ['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress', 'email', 'mail'],
            'first_name': ['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname', 'givenName', 'firstName'],
            'last_name': ['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname', 'sn', 'surname', 'lastName'],
            'display_name': ['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name', 'displayName', 'cn'],
            ...
            for saml_attr in saml_attrs:
                if saml_attr in attributes and attributes[saml_attr]:
                    ...
                    if isinstance(value, list) and value:
                        user_data[f'external_{user_field}'] = value[0] if user_field != 'groups' else value
    ...
    async def _find_or_create_sso_user(
        ...
        user = User(
            ...
            hashed_password=get_password_hash("sso_user_no_password"),  # SSO users don't need passwords
            is_active=True,
            is_sso_user=True
        )
        db.add(user)
        ...
        logger.info(f"Created new SSO user: {external_user_id} for provider: {provider.name}")
        return sso_user
    ...
    def _delete_saml_session(self, db: Session, nameid: str):
        ...
        try:
            ...
            sso_sessions = db.query(SSOSession).join(SSOUser).filter(
                SSOUser.external_user_id == nameid,
                SSOSession.active == True
            ).all()
