...
from .models import (
    ...
    AggregatedMetric, InsightReport, MetricType, AggregationPeriod
)
...
class InsightsEngine:
                ...
                if cost_percentage > 40:
                    insights.append(Insight(
                        ...
                        metadata={
                            ...
                            "avg_cost_per_request": float(result.avg_cost_per_request)
                        }
                    ))
    ...
    async def _analyze_unused_capacity(
        ...
        try:
            ...
            if len(daily_usage) > 7:  # Need at least a week of data
                ...
                std_daily_requests = statistics.stdev(request_counts) if len(request_counts) > 1 else 0
    ...
    async def _analyze_token_efficiency(
        ...
        try:
            ...
            ).group_by(CostMetric.service, CostMetric.model).all()
            ...
            for result in token_analysis:
                ...
                if result.avg_input_tokens and result.avg_output_tokens:
                    input_output_ratio = result.avg_input_tokens / result.avg_output_tokens
                    
                    if input_output_ratio > 5:  # >5:1 ratio suggests inefficient prompts
                        insights.append(Insight(
                            ...
                            metadata={
                                ...
                                "avg_output_tokens": float(result.avg_output_tokens)
                            }
                        ))
    ...
    async def _analyze_cost_trends(
        ...
        try:
            ...
            ).group_by(func.date(CostMetric.timestamp)).order_by('date').all()
            ...
            if len(daily_costs) >= 7:  # Need at least a week of data
                ...
                if len(costs) > 1:
                    ...
                    if slope > 0.1:  # $0.10 increase per day
                        ...
                        current_monthly = sum(costs[-7:]) * 30 / 7  # Estimate based on last week
                        
                        insights.append(Insight(
                            title="Rising Cost Trend Detected",
                            description=f"Costs are increasing by approximately ${slope:.2f} per day",
                            impact_score=min(monthly_increase / current_monthly * 100, 100),
                            confidence_score=70,
                            category="cost_optimization",
    ...
    async def _analyze_slow_endpoints(
        ...
        try:
            ...
            overall_avg = self.db.query(func.avg(PerformanceMetric.response_time_ms)).filter(
                and_(
                    PerformanceMetric.timestamp >= start_date,
                    PerformanceMetric.timestamp <= end_date,
                    PerformanceMetric.success == True
    ...
    async def _analyze_error_rates(
        ...
        try:
            ...
            results = query.group_by(PerformanceMetric.component)\
                          .having(func.count(PerformanceMetric.id) >= 10)\
                          .all()
            ...
            for result in results:
                ...
                if result.avg_memory and result.avg_memory > 1000:  # >1GB average
                    insights.append(Insight(
                        ...
                        impact_score=min(result.avg_memory / 50, 100),  # Scale to 100
                        confidence_score=85,
                        category="performance",
