...
class LLMGenerationService:
    ...
    def __init__(self):
        self.llm = load_llm()
        logger.info(f"LLM Generation Service initialized with {type(self.llm).__name__}")
    ...
    async def generate_enhanced_rfc_content(
        ...
        for section_key, section_description in sections_to_generate:
            try:
                ...
                enhanced_content[section_key] = template_vars.get(section_key, f"## {section_description}\n\nКонтент будет сгенерирован...")
...
"""
        ...
        prompt = prompts.get(section_key, f"""
    ...
    def _get_fallback_questions(self, task_type: TaskType) -> List[Dict[str, Any]]:
        ...
        base_questions = [
            ...
            },
            {
                ...
                "options": ["Малый (< 1K пользователей)", "Средний (1K-10K)", "Большой (10K-100K)", "Очень большой (> 100K)"],
                "is_required": True,
...
"""
        ...
        prompt = documentation_prompts.get(documentation_type, f"""
...
""")
        ...
        try:
            response = await self.llm.generate(prompt)
            logger.info(f"Generated {documentation_type} documentation: {len(response)} characters")
            return response.strip()
            
        except Exception as e:
            logger.error(f"Failed to generate {documentation_type} documentation with LLM: {e}")
