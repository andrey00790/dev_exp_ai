...
from .models import SSOProvider, SSOUser, SSOSession, SSOProviderType, DEFAULT_PROVIDER_CONFIGS
...
class OAuthAuthHandler:
    ...
    async def initiate_login(self, provider: SSOProvider, state: Optional[str] = None) -> Tuple[str, str]:
        ...
        if provider.provider_type in [SSOProviderType.OAUTH_GOOGLE, SSOProviderType.OAUTH_MICROSOFT]:
    ...
    async def handle_callback(
        ...
        try:
            ...
            user_profile = await self._get_user_profile(provider, config, token_data['access_token'])
            ...
            logger.info(f"OAuth authentication successful for user: {user_data['external_user_id']}")
    ...
    async def refresh_token(self, provider: SSOProvider, refresh_token: str) -> Dict[str, Any]:
    ...
    async def revoke_token(self, provider: SSOProvider, token: str, token_type: str = 'access_token'):
        """Revoke OAuth token"""
        config = self._get_provider_config(provider)
    ...
    async def _get_user_profile(self, provider: SSOProvider, config: Dict[str, Any], access_token: str) -> Dict[str, Any]:
    ...
    def _map_oauth_profile(self, provider: SSOProvider, profile: Dict[str, Any], token_data: Dict[str, Any]) -> Dict[str, Any]:
        """Map OAuth profile to user data"""
        user_data = {
        ...
        if provider.provider_type == SSOProviderType.OAUTH_GOOGLE:
            ...
            user_data.update({
                'external_user_id': profile.get('id') or profile.get('sub'),
                'external_email': profile.get('mail') or profile.get('userPrincipalName'),
            ...
            user_data.update({
                'external_user_id': profile.get('id') or profile.get('sub') or profile.get('user_id'),
                'external_email': profile.get('email'),
                'external_name': profile.get('name') or profile.get('display_name'),
                'external_username': profile.get('username') or profile.get('login')
            })
    ...
    async def _find_or_create_sso_user(
        ...
        sso_user = db.query(SSOUser).filter(
        ...
        user = User(
            email=user_data.get('external_email', f"{external_user_id}@{provider.name}"),
        ...
        logger.info(f"Created new OAuth SSO user: {external_user_id} for provider: {provider.name}")
        return sso_user
    
    async def _create_sso_session(self, db: Session, sso_user: SSOUser, token_data: Dict[str, Any]):
