...
class StressEnduranceTester:
    ...
    async def run_escalating_load_test(
        self, 
        max_users: int = 200, 
        escalation_steps: int = 10,
        step_duration: int = 30
    ) -> StressTestResult:
        """Test with escalating load to find breaking point"""
        logger.info(f"🔥 Running escalating load test: 0 → {max_users} users in {escalation_steps} steps")
        ...
        for step in range(1, escalation_steps + 1):
            ...
            logger.info(f"🚀 Step {step}/{escalation_steps}: {current_users} concurrent users")
            ...
            step_rps = step_requests / step_duration_actual if step_duration_actual > 0 else 0.0
            ...
            error_rate = (step_failures / step_requests * 100) if step_requests > 0 else 0
            if error_rate > 50 or step_rps < (peak_rps * 0.3):
                system_degraded = True
                logger.warning(f"⚠️ System degradation detected at {current_users} users")
            
            logger.info(f"✅ Step {step} complete: {step_rps:.1f} RPS, {error_rate:.1f}% error rate")
        ...
        if recovery_rate < 80:
            logger.warning(f"⚠️ Poor recovery: {recovery_rate:.1f}% success after stress")
        ...
        result = StressTestResult(
            ...
            cpu_usage_avg=statistics.mean(cpu_measurements) if cpu_measurements else 0.0,
        ...
        logger.info(f"✅ Escalating load test completed: Peak {peak_rps:.1f} RPS at {max_users} users")
    ...
    async def run_sustained_load_test(
        ...
        if memory_leak_detected:
            logger.warning(f"⚠️ Potential memory leak detected: {memory_growth:.1f}MB growth")
        ...
        self.results.append(result)
        logger.info(f"✅ Sustained load test completed: {avg_rps:.1f} avg RPS over {duration_minutes} minutes")
    ...
    async def run_spike_load_test(
        ...
        logger.info(f"⚡ Running spike load test: {baseline_users} → {spike_users} users spike")
        ...
        spike_rps = (total_requests * 0.8) / spike_duration_actual  # Estimate spike portion
        peak_rps = spike_rps
        ...
        self.results.append(result)
        logger.info(f"✅ Spike load test completed: {peak_rps:.1f} peak RPS, {recovery_rate:.1f}% recovery")
    ...
    def generate_stress_test_report(self) -> str:
        ...
        for result in self.results:
            success_rate = (result.successful_requests / result.total_requests * 100) if result.total_requests > 0 else 0.0
        ...
        if degraded_tests:
            ...
            report_lines.append(f"- System degradation begins around **{min_degraded_users} concurrent users**")
        else:
            report_lines.append(f"- No system degradation detected up to **{max_users_tested} concurrent users** ✅")
        
        # Memory analysis
        max_memory_growth = max(r.memory_end_mb - r.memory_start_mb for r in self.results)
        if max_memory_growth > 50:
            report_lines.append(f"- Potential memory leak detected: **{max_memory_growth:.1f} MB** growth ⚠️")
        else:
            report_lines.append(f"- Memory usage stable: **{max_memory_growth:.1f} MB** max growth ✅")
        
        # CPU analysis
        max_cpu_usage = max(r.cpu_usage_peak for r in self.results)
        if max_cpu_usage > 80:
            report_lines.append(f"- High CPU usage detected: **{max_cpu_usage:.1f}%** peak ⚠️")
        else:
            report_lines.append(f"- CPU usage acceptable: **{max_cpu_usage:.1f}%** peak ✅")
...
class TestStressScenarios:
    ...
    async def test_escalating_load_moderate(self):
        ...
        result = await tester.run_escalating_load_test(max_users=50, escalation_steps=5, step_duration=15)
    ...
    async def test_sustained_load_short(self):
        ...
        tester = StressEnduranceTester()
        result = await tester.run_sustained_load_test(concurrent_users=25, duration_minutes=2)
    ...
    async def test_spike_load_moderate(self):
        ...
        tester = StressEnduranceTester()
        result = await tester.run_spike_load_test(baseline_users=5, spike_users=30, spike_duration=20)
...
class TestExtremeStress:
    ...
    async def test_comprehensive_stress_suite(self):
        ...
        escalating_result = await tester.run_escalating_load_test(max_users=100, escalation_steps=8, step_duration=20)
        sustained_result = await tester.run_sustained_load_test(concurrent_users=40, duration_minutes=5)
        spike_result = await tester.run_spike_load_test(baseline_users=10, spike_users=60, spike_duration=25)
        ...
        assert overall_success_rate > 60, f"Overall system success rate too low: {overall_success_rate:.1f}%"
        assert total_requests > 1000, f"Insufficient load tested: {total_requests} requests"
        
        print(f"🎯 OVERALL SYSTEM ASSESSMENT: {overall_success_rate:.1f}% success rate across {total_requests:,} requests")
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        escalating = await tester.run_escalating_load_test(max_users=80, escalation_steps=6)
        sustained = await tester.run_sustained_load_test(concurrent_users=30, duration_minutes=3)
