...
class ModelTrainingRequest(BaseModel):
    ...
    training_type: str = Field(
        ..., description="Тип обучения: 'full', 'incremental', 'multilingual'"
    )
...
class SearchOptimizationRequest(BaseModel):
    ...
    test_queries: Optional[List[str]] = Field(
        None, description="Тестовые запросы для измерения"
    )
    optimization_type: str = Field(
        "full", description="Тип оптимизации: 'performance', 'quality', 'full'"
    )
...
async def train_model(
    ...
    try:
        ...
        duration_map = {"full": 60, "incremental": 20, "multilingual": 30}
        estimated_duration = duration_map.get(request.training_type, 30)

        # Запускаем обучение в фоне
        if request.training_type == "full":
            background_tasks.add_task(
                run_full_training_pipeline, trainer, training_id, request.use_feedback
            )
        elif request.training_type == "multilingual":
            background_tasks.add_task(
                run_multilingual_optimization, trainer, training_id
            )
        else:
            background_tasks.add_task(
                run_incremental_training, trainer, training_id, request.use_feedback
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка запуска обучения: {str(e)}"
        )
...
async def get_training_status(
    training_id: str, current_user: dict = Depends(get_current_user)
):
...
async def analyze_rfc_quality(
    request: RFCQualityAnalysisRequest, current_user: dict = Depends(get_current_user)
):
    """
    ...
    """
    try:
        from domain.rfc_generation.rfc_quality_enhancer import \
            get_rfc_quality_enhancer
        ...
        metrics = quality_enhancer.analyze_rfc_quality(
            content=request.rfc_content, title=request.rfc_title
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка анализа качества: {str(e)}"
        )
...
async def optimize_search(
    request: SearchOptimizationRequest, current_user: dict = Depends(get_current_user)
):
    """
    ...
    """
    try:
        from domain.integration.vector_search_optimizer import \
        ...
        cache_stats = {
            ...
            "cache_hit_rate": (
                optimizer.cache_stats["hits"]
                / (optimizer.cache_stats["hits"] + optimizer.cache_stats["misses"])
                if (optimizer.cache_stats["hits"] + optimizer.cache_stats["misses"]) > 0
                else 0.0
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка оптимизации поиска: {str(e)}"
        )


@router.get("/status")
@rate_limit
async def get_ai_enhancement_status(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        try:
            ...
            components_status["model_trainer"] = {"status": "error", "error": str(e)}
        ...
        # Общий статус
        all_available = all(
            comp["status"] == "available" for comp in components_status.values()
        )
        ...
        return {
            ...
            "capabilities": {
                "model_training": components_status.get("model_trainer", {}).get(
                    "status"
                )
                == "available",
                "rfc_quality_analysis": components_status.get(
                    "rfc_quality_enhancer", {}
                ).get("status")
                == "available",
                "search_optimization": components_status.get(
                    "vector_search_optimizer", {}
                ).get("status")
                == "available",
...
async def run_multilingual_optimization(trainer, training_id: str):
    ...
    try:
        ...
        logger.info(
            f"Multilingual optimization completed: {training_id}, result: {result}"
        )
    except Exception as e:
