...
class SystemLoadE2ETester:
    ...
    async def run_concurrent_load_test(
        ...
        successful_requests = sum(
            1 for r in all_results if r["success"] and r["status"] == 200
        )
        failed_requests = total_requests - successful_requests

        # Response time statistics
        successful_durations = [
            r["duration"] for r in all_results if r["success"] and r["status"] == 200
        ]
        ...
        if successful_durations:
            ...
            p95_response_time = statistics.quantiles(successful_durations, n=20)[
                18
            ]  # 95th percentile
        else:
        ...
        return (
            result.error_rate < 5.0 and result.avg_response_time < 2.0
        )  # Success criteria
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                workflows = [
                    ...
                    (
                        ...
                        "POST",
                        {"query": f"user {user_id} search query", "limit": 3},
                    ),
                    ("llm/providers", "GET", {}),
                    ("auth/budget", "GET", {}),
                ]
    ...
    def generate_load_test_report(self):
        ...
        print(
            f"   Average RPS: {sum(r.requests_per_second for r in self.results) / len(self.results):.2f}"
        )
        ...
        for result in self.results:
            status = (
                "✅ PASS"
                if result.error_rate < 5.0 and result.avg_response_time < 2.0
                else "❌ FAIL"
            )
            print(f"\n{status} {result.test_name}")
            print(
                f"   Requests: {result.total_requests:,} (Success: {result.successful_requests:,})"
            )
        ...
        print(
            f"   ✅ Response time < 2s: {sum(1 for r in self.results if r.avg_response_time < 2.0)}/{len(self.results)}"
        )
        print(
            f"   ✅ Error rate < 5%: {sum(1 for r in self.results if r.error_rate < 5.0)}/{len(self.results)}"
        )
        print(
            f"   ✅ RPS > 10: {sum(1 for r in self.results if r.requests_per_second > 10)}/{len(self.results)}"
        )

        # Performance classification
        if overall_success_rate > 98 and all(
            r.avg_response_time < 1.0 for r in self.results
        ):
            print(f"\n🏆 PERFORMANCE GRADE: EXCELLENT")
        elif overall_success_rate > 95 and all(
            r.avg_response_time < 2.0 for r in self.results
        ):
