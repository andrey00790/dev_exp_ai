...
class TestServicesIntegration:
    ...
    def test_document_crud_operations(self):
        ...
        # Получение документа
        get_response = requests.get(f"{BASE_URL}/api/v1/documents/{document_id}", timeout=10)
        ...
        # Удаление документа
        delete_response = requests.delete(f"{BASE_URL}/api/v1/documents/{document_id}", timeout=10)
        assert delete_response.status_code in [200, 204]  # API может возвращать 200 или 204
        
        # Проверка удаления
        get_deleted_response = requests.get(f"{BASE_URL}/api/v1/documents/{document_id}", timeout=10)
    ...
    def test_system_performance_baseline(self):
        ...
        logger.info(f"✅ System performance baseline passed (response time: {response_time:.2f}s)")
...
class TestServiceAvailability:
    ...
    def test_all_services_available(self):
        ...
        for service_name, is_available in results.items():
            ...
            logger.info(f"{status} {service_name}: {'Available' if is_available else 'Not available'}")
