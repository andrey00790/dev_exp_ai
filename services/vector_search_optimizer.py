...
from services.vector_search_service import get_vector_search_service, SearchRequest
from vectorstore.collections import get_collection_manager
...
class VectorSearchOptimizer:
    ...
    def _cache_results(self, cache_key: str, results: List[Dict[str, Any]], search_time: float):
    ...
    async def measure_search_performance(self, test_queries: List[str]) -> SearchPerformanceMetrics:
        ...
        success_rate = (len(test_queries) - failed_queries) / len(test_queries) if test_queries else 0.0
        ...
        metrics = SearchPerformanceMetrics(
            ...
            embedding_generation_time_ms=np.mean(embedding_times) if embedding_times else 0.0,
            vector_search_time_ms=np.mean(search_times) if search_times else 0.0,
            post_processing_time_ms=avg_response_time - np.mean(search_times) if search_times else 0.0,
        ...
        logger.info(f"Performance measurement completed: {avg_response_time:.2f}ms avg, {queries_per_second:.2f} QPS")
        return metrics
