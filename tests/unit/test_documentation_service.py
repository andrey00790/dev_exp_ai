...
from domain.integration.documentation_service import DocumentationService
from models.documentation import (CodeAnalysisRequest, CodeFile, CodeLanguage,
...
"""
    ...
    async def test_analyze_python_code(self, documentation_service, sample_python_code):
        ...
        calc_sum_func = next(
            f for f in analysis.functions if f["name"] == "calculate_sum"
        )
    ...
    async def test_analyze_code_string(self, documentation_service, sample_python_code):
        ...
        language = documentation_service._detect_language_from_content(
            sample_python_code
        )
    ...
    def test_detect_language_from_content(self, documentation_service):
        ...
        assert (
            documentation_service._detect_language_from_content(python_code)
            == CodeLanguage.PYTHON
        )
        ...
        assert (
            documentation_service._detect_language_from_content(js_code)
        ...
        assert (
            documentation_service._detect_language_from_content(java_code)
            == CodeLanguage.JAVA
        )
    ...
    async def test_generate_documentation_with_llm_fallback(
        ...
        mock_instance.generate_code_documentation.side_effect = Exception(
            "LLM service error"
        )
