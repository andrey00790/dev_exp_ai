...
class TestDataSyncScheduler:
    ...
    async def test_initialize_success(self, temp_config_file):
        ...
        ) as mock_init_components, patch.object(
            scheduler, "_setup_sync_jobs"
        ) as mock_setup_jobs:
    ...
    async def test_create_default_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                len(config["sync_jobs"]) >= 4
            )  # confluence, gitlab, jira, local_files
    ...
    async def test_initialize_components_success(self, mock_scheduler):
        ...
        with patch("builtins.open", mock_open(read_data=yaml.dump(main_config))), patch(
            "core.cron.data_sync_scheduler.DatabaseManager"
        ) as MockDB, patch(
            "core.cron.data_sync_scheduler.VectorStoreManager"
        ) as MockVector, patch(
            "core.cron.data_sync_scheduler.ContentProcessor"
        ) as MockProcessor:
    ...
    async def test_sync_confluence_success(self, mock_scheduler):
        ...
        with patch(
            "core.cron.data_sync_scheduler.ConfluenceIngestionClient"
        ) as MockClient:
            mock_client = Mock()
            ...
            mock_scheduler.content_processor.process_batch = AsyncMock(
                return_value=[
                    {"id": "1", "title": "Test Page", "content": "Test content"}
                ]
            )
    ...
    async def test_sync_gitlab_success(self, mock_scheduler):
        ...
        with patch("core.cron.data_sync_scheduler.GitLabIngestionClient") as MockClient:
            ...
            mock_scheduler.content_processor.process_batch = AsyncMock(
                return_value=[{"path": "README.md", "content": "# Project"}]
            )
            mock_scheduler.db_manager.save_documents = AsyncMock(return_value=1)
            mock_scheduler.vector_store.store_embeddings = AsyncMock()
    ...
    async def test_sync_local_files_success(self, mock_scheduler):
        ...
        with patch(
            "core.cron.data_sync_scheduler.LocalFilesProcessor"
        ) as MockProcessor, patch("core.cron.data_sync_scheduler.Path") as MockPath:

            # Исправлено: правильно мокаем Path
            mock_path_instance = Mock()
            MockPath.return_value = mock_path_instance

            mock_processor = Mock()
            # Исправлено: process_files_batches теперь возвращает итератор
            mock_processor.process_files_batches = AsyncMock()
            mock_processor.process_files_batches.return_value = [
                [{"path": "guide.pdf", "content": "Training content"}]
            ]
            MockProcessor.return_value = mock_processor

            mock_scheduler.content_processor.process_batch = AsyncMock(
                return_value=[{"path": "guide.pdf", "content": "Training content"}]
            )
            mock_scheduler.db_manager.save_documents = AsyncMock(return_value=1)
            mock_scheduler.vector_store.store_embeddings = AsyncMock()
    ...
    async def test_update_job_config_success(self, mock_scheduler):
        ...
        new_config = {"enabled": False, "schedule": "0 6 * * *", "incremental": False}
...
class TestSyncSchedulerIntegration:
    ...
    async def test_full_lifecycle(self, temp_config_file):
        ...
        with patch.object(
            ...
            scheduler.scheduler.get_job.return_value = Mock(
            ...
            result = await scheduler.update_job_config(
                "confluence_main_confluence", {"enabled": False}
            )
            assert result is True
