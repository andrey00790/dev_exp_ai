...
class RealtimeMonitoringService:
        ...
        for sla in self.slas.values():
            ...
            for threshold in sla.thresholds:
                ...
                for dp in recent_data:
                    if threshold.comparison == "lt" and dp.value >= threshold.threshold_value:
                        violations += 1
                    elif threshold.comparison == "gt" and dp.value <= threshold.threshold_value:
                        violations += 1
                    elif threshold.comparison == "eq" and dp.value != threshold.threshold_value:
                        violations += 1
                
                violation_percent = (violations / total_points) * 100
                
                if violation_percent > threshold.violation_threshold_percent:
                    await self._create_sla_violation_alert(sla, threshold, violation_percent, data_point)
    ...
    async def _create_threshold_alert(
        ...
        for existing_alert in self.alerts.values():
            if (existing_alert.metric == data_point.metric and 
                existing_alert.source == data_point.source and 
        ...
        alert = Alert(
            ...
            metadata={
                ...
                "exceeded_by_percent": ((data_point.value - threshold) / threshold) * 100
            }
        )
    ...
    async def _handle_anomaly(self, anomaly: Anomaly):
        ...
        logger.info(f"Anomaly detected: {anomaly.description} (confidence: {anomaly.confidence:.2f})")
        ...
        for alert in self.alerts.values():
            ...
            if (current_time - alert.created_at).total_seconds() > 3600:
                ...
                if recent_data:
                    ...
                    if alert.metric in [MonitoringMetric.RESPONSE_TIME, MonitoringMetric.ERROR_RATE]:
                        if avg_recent < alert.threshold_value * 0.8:  # 20% below threshold
    ...
    def get_sla_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for sla_id, sla in self.slas.items():
            ...
            for threshold in sla.thresholds:
                ...
                recent_alerts = [
                    alert for alert in self.alerts.values()
                    if (alert.metric == threshold.metric and
                        "sla_id" in (alert.metadata or {}) and
                        alert.metadata["sla_id"] == sla_id and
                ...
                if len(recent_alerts) > 0:
                    ...
                    status["current_compliance"] = min(status["current_compliance"], 95.0)
