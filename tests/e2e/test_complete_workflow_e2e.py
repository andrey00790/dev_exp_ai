...
class CompleteWorkflowE2ETester:
    ...
    async def test_e2e_authentication_workflow(self):
        ...
        try:
            ...
            async with self.session.get(
                f"{self.backend_url}/api/v1/auth/verify", headers=headers
            ) as response:
                assert response.status == 200
            ...
            # Test user info
            async with self.session.get(
                f"{self.backend_url}/api/v1/auth/me", headers=headers
            ) as response:
                assert response.status == 200
            ...
            # Test budget info
            async with self.session.get(
                f"{self.backend_url}/api/v1/auth/budget", headers=headers
            ) as response:
                assert response.status == 200
            ...
            duration = time.time() - start_time
            await self.add_test_result(
                "E2E Authentication Workflow", "FAILED", duration, {"error": str(e)}
            )
            return False
    ...
    async def test_e2e_vector_search_workflow(self):
        ...
        try:
            ...
            await self.add_test_result(
                ...
                duration,
                {
                    ...
                    "avg_search_time_ms": sum(
                        r["search_time_ms"] for r in search_results
                    )
                    / len(search_results),
            ...
            duration = time.time() - start_time
            await self.add_test_result(
                "E2E Vector Search Workflow", "FAILED", duration, {"error": str(e)}
            )
            return False
    ...
    async def test_e2e_llm_operations_workflow(self):
        ...
        try:
            ...
            # Test LLM health
            async with self.session.get(
                f"{self.backend_url}/api/v1/llm/health", headers=self.get_auth_headers()
            ) as response:
            ...
            # Test LLM statistics
            async with self.session.get(
                f"{self.backend_url}/api/v1/llm/stats", headers=self.get_auth_headers()
            ) as response:
            ...
            await self.add_test_result(
                ...
                duration,
                {
                    ...
                    "rfc_generation": (
                        "available" if rfc_status == 200 else "unavailable"
                    ),
                    "documentation_generation": (
                        "available" if doc_status == 200 else "unavailable"
                    ),
                    "qa_system": "available" if qa_status == 200 else "unavailable",
                    "stats_endpoint": (
                        "available" if stats_status == 200 else "unavailable"
                    ),
                },
            )
            return True
            ...
            duration = time.time() - start_time
            await self.add_test_result(
                "E2E LLM Operations Workflow", "FAILED", duration, {"error": str(e)}
            )
            return False
    ...
    async def test_e2e_cache_system_workflow(self):
        ...
        try:
            ...
            duration = time.time() - start_time
            await self.add_test_result(
                "E2E Cache System Workflow", "FAILED", duration, {"error": str(e)}
            )
            return False
    ...
    async def test_e2e_performance_workflow(self):
        ...
        try:
            ...
            successful_requests = sum(
                1 for result in results if result["status"] == 200
            )
            avg_response_time = sum(result["duration"] for result in results) / len(
                results
            )
            ...
            duration = time.time() - start_time
            await self.add_test_result(
                "E2E Performance Workflow", "FAILED", duration, {"error": str(e)}
            )
            return False
    ...
    async def test_e2e_security_workflow(self):
        ...
        try:
            ...
            async with self.session.get(
                f"{self.backend_url}/api/v1/auth/me", headers=invalid_headers
            ) as response:
                assert response.status == 401
            ...
            async with self.session.get(
                f"{self.backend_url}/api/v1/auth/me", headers=malformed_headers
            ) as response:
                assert response.status == 401
            ...
            async with self.session.get(
                f"{self.backend_url}/api/v1/auth/me", headers=self.get_auth_headers()
            ) as response:
                assert response.status == 200
            ...
            duration = time.time() - start_time
            await self.add_test_result(
                "E2E Security Workflow", "FAILED", duration, {"error": str(e)}
            )
            return False
