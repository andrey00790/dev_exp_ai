...
async def get_llm_health():
    ...
    try:
        ...
        logger.info(f"✅ LLM health check completed: {health_data.get('healthy_count', 0)}/{health_data.get('total_count', 0)} providers healthy")
...
async def generate_text(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Generation failed after retries: {str(e)}")
    except Exception as e:
...
async def generate_documentation(
    ...
    try:
        ...
        logger.info(f"✅ Documentation generation completed in {response_time:.2f}s for {request.language} code")
...
async def check_all_providers(
    ...
    try:
        ...
        health_results = await with_timeout(
            llm_service.health_check(),
            AsyncTimeouts.LLM_REQUEST * 1.5,  # 90 seconds for comprehensive health check
...
async def _generate_single_text_with_context(llm_service, req: TextGenerationRequest, index: int) -> Dict[str, Any]:
