...
async def verify_token(token: str) -> TokenData:
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail=f"Token verification timed out: {str(e)}")
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if (request.url.path in EXCLUDED_PATHS or 
            request.url.path.startswith("/api/v1/auth/token")):
    ...
    def _create_auth_error_response(self, detail: str, status_code: int = 401) -> Response:
...
async def create_user(user_data: UserCreate) -> User:
    ...
    try:
        ...
        create_background_task(
            _log_user_creation(user_data.email),
            f"user_creation_log_{user_data.email}"
        )
...
async def get_auth_stats() -> Dict[str, Any]:
    ...
    if total_validations > 0:
        stats["token_success_rate"] = ((total_validations - stats["token_failures"]) / total_validations) * 100
    else:
...
async def health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        # Process results
        health["jwt_system"] = jwt_test if not isinstance(jwt_test, Exception) else {"status": "failed", "error": str(jwt_test)}
        health["user_database"] = db_test if not isinstance(db_test, Exception) else {"status": "failed", "error": str(db_test)}
        health["password_hashing"] = pwd_test if not isinstance(pwd_test, Exception) else {"status": "failed", "error": str(pwd_test)}
        health["token_validation"] = token_test if not isinstance(token_test, Exception) else {"status": "failed", "error": str(token_test)}
        ...
        all_healthy = all(
            ...
            for component in [health["jwt_system"], health["user_database"], health["password_hashing"], health["token_validation"]]
