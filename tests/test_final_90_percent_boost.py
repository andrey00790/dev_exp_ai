                    ...
                    for item_name in module_items:
                        try:
                                ...
                                try:
                                    ...
                                    try:
                                        if asyncio.iscoroutinefunction(item):
                                            ...
                                            result = item()
                                        print(f"✅ {module_name}.{item_name}: выполнена без параметров")
                                    ...
                                    for method_name in methods[:10]:
                                        try:
                                            ...
                                            if asyncio.iscoroutinefunction(method):
                                                result = asyncio.run(method(**method_params))
                                            else:
                                            ...
                                            try:
                                                if asyncio.iscoroutinefunction(method):
                                                    ...
                                                    result = method()
                                                print(f"✅ {item_name}.{method_name}: выполнен без параметров")
                                            except Exception as e2:
                                                print(f"⚠️ {item_name}.{method_name}: {str(e2)[:50]}")
                            ...
                            # Если это константа
                                ...
                                try:
                                    value = item
                                    print(f"✅ {module_name}.{item_name}: константа обработана")
                                except Exception as e:
                                    ...
                                    if hasattr(item, '__tablename__'):
                                            ...
                                            if not attr_name.startswith('_') and not callable(getattr(instance, attr_name)):
                                                try:
                                                    if 'id' in attr_name.lower():
                                                        setattr(instance, attr_name, 'test_id_123')
                                                    elif 'name' in attr_name.lower() or 'title' in attr_name.lower():
                                                        setattr(instance, attr_name, 'Test Name')
                                                    elif 'content' in attr_name.lower() or 'text' in attr_name.lower():
                                                        setattr(instance, attr_name, 'Test content text')
                                                    elif 'email' in attr_name.lower():
                                                        setattr(instance, attr_name, 'test@example.com')
                                                        ...
                                                        setattr(instance, attr_name, 'http://test.com')
                                                    elif 'status' in attr_name.lower():
                                                        setattr(instance, attr_name, 'active')
                                                    else:
                                                        setattr(instance, attr_name, 'test_value')
                                        ...
                                        for method_name in methods[:15]:
                                            try:
                                                ...
                                                if asyncio.iscoroutinefunction(method):
                                                    result = asyncio.run(method(**model_params))
                                                else:
        ...
        with patch('sqlalchemy.orm.Session') as mock_session, \
            ...
            for module_name in scripts_modules:
                try:
                    ...
                    script_functions = [item for item in dir(module) 
                    ...
                    for func_name in script_functions:
                        try:
                            ...
                            script_params = {
                                ...
                                'documents': [
                                    {'id': 'doc1', 'title': 'Test Doc 1', 'content': 'Content 1'},
                                    {'id': 'doc2', 'title': 'Test Doc 2', 'content': 'Content 2'}
                                ],
                            ...
                            try:
                                if asyncio.iscoroutinefunction(func):
                                    ...
                                    result = func()
                                print(f"✅ {module_name}.{func_name}: скрипт выполнен без параметров")
                            except Exception as e2:
                ...
                for item_name in module_items:
                    try:
                        ...
                        if callable(item) and not isinstance(item, type):
                            ...
                            if item_name == 'main':
                                print(f"⚠️ {module_name}.{item_name}: пропущена (argparse)")
                                continue
                            ...
                            try:
                                ...
                                print(f"✅ {module_name}.{item_name}: utility функция выполнена")
                        ...
                        # Если это константа
                            ...
                            try:
                                ...
                                if hasattr(item, '__module__') and item.__module__ == 'typing':
                                    print(f"⚠️ {module_name}.{item_name}: Type {item.__name__} cannot be instantiated; use {item.__name__.lower()}() instead")
                                elif item_name in ['argparse', 'json']:
                                    print(f"✅ {module_name}.{item_name}: utility константа обработана")
                                else:
                                    value = item
                                    print(f"✅ {module_name}.{item_name}: utility константа обработана")
                            except Exception as e:
                        ...
                        try:
                                    ...
                                    for method_name in methods[:15]:
                                        try:
                                            ...
                                            core_params = {
                                                ...
                                                'job_id': 'test_job_123',
                                                'task_data': {'type': 'sync', 'priority': 'high'},
                                                'retry_count': 0,
                                                'max_retries': 3,
                                                'timeout': 300
                                            }
                                            ...
                                            if asyncio.iscoroutinefunction(method):
                                                result = asyncio.run(method(**core_params))
                                            else:
                            ...
                            # Если это функция
                                ...
                                try:
                                    ...
                                    print(f"✅ {module_name}.{item_name}: core функция выполнена")
