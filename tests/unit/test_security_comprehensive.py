...
class CostControlManager:
    ...
    async def check_budget(self, user_id, cost):
        ...
        budget = self.budgets.get(user_id, {"budget_limit": 100.0, "current_usage": 0.0})
...
class TestAuthService:
    ...
    async def test_authenticate_user_success(self, mock_verify, mock_get_user, auth_service, sample_user):
    ...
    @patch('app.security.auth.get_user_by_email')
    @patch('app.security.auth.verify_password')
    async def test_authenticate_user_wrong_password(self, mock_verify, mock_get_user, auth_service, sample_user):
    ...
    async def test_create_user(self, mock_create, mock_hash, mock_get_user, auth_service):
        """Test user creation"""
        # Setup mocks
    ...
    @patch('app.security.auth.get_user_by_email')
    async def test_create_user_already_exists(self, mock_get_user, auth_service, sample_user):
...
class TestCostControlManager:
    ...
    async def test_check_budget_within_limit(self, mock_get_budget, cost_control_manager):
    ...
    @patch('app.security.cost_control.get_user_budget')
    async def test_check_budget_exceeds_limit(self, mock_get_budget, cost_control_manager):
    ...
    async def test_get_usage_stats(self, mock_get_history, cost_control_manager):
        ...
        mock_get_history.return_value = [
            ...
            {"operation": "generate", "cost": 25.0, "timestamp": "2024-01-15T11:00:00Z"}
        ]
...
class TestOAuthAuthenticator:
    ...
    @patch('app.security.sso.oauth_auth.requests.get')
    def test_get_authorization_url(self, mock_get, oauth_authenticator):
    ...
    @patch('app.security.sso.oauth_auth.requests.post')
    def test_exchange_code_for_token(self, mock_post, oauth_authenticator):
    ...
    @patch('app.security.sso.oauth_auth.requests.get')
    def test_get_user_info(self, mock_get, oauth_authenticator):
...
class TestSecurityIntegration:
    ...
    async def test_full_authentication_flow(self, auth_service, token_manager):
        ...
        with patch('app.security.auth.get_user_by_email') as mock_get_user, \
             patch('app.security.auth.verify_password') as mock_verify:
            ...
            # Authenticate user
            auth_result = await auth_service.authenticate_user("test@example.com", "password")
    ...
    async def test_cost_control_integration(self, cost_control_manager, auth_service):
        ...
        with patch('app.security.cost_control.get_user_budget') as mock_budget, \
             patch('app.security.cost_control.update_user_usage') as mock_update:
            ...
            can_perform = await cost_control_manager.check_budget(user_id, operation_cost)
...
class TestSecurityAPI:
    ...
    @patch('app.api.v1.auth.AuthService')
    def test_register_endpoint(self, mock_auth_service, client):
        """Test user registration endpoint"""
        # Setup mock
        mock_auth_instance = Mock()
        mock_auth_instance.create_user = AsyncMock(return_value={
            "user_id": "new_user",
            "email": "newuser@example.com"
        })
        mock_auth_service.return_value = mock_auth_instance
    ...
    @patch('app.api.v1.auth.CostControlManager')
    def test_budget_status_endpoint(self, mock_cost_control, mock_auth, client):
        """Test budget status endpoint"""
        # Setup mocks
        mock_auth.return_value = {"user_id": "test_user"}
        
        mock_cost_instance = Mock()
        mock_cost_instance.get_usage_stats = AsyncMock(return_value={
            "total_cost": 75.0,
            "budget_limit": 100.0,
            "remaining_budget": 25.0
        })
        mock_cost_control.return_value = mock_cost_instance
        
        # Make request
        response = client.get("/api/v1/auth/budget/status")
