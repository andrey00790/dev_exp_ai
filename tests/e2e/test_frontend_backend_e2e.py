...
class FrontendBackendE2ETester:
    ...
    async def test_api_cors_configuration(self):
        ...
        try:
            ...
            async with self.session.options(
                f"{self.backend_url}/api/v1/auth/login", headers=headers
            ) as response:
                    ...
                    async with self.session.post(
                        ...
                        if search_response.status == 200:
                            ...
                            for field in search_fields:
                                assert (
                                    field in search_data
                                ), f"Missing search field: {field}"

                    # Test LLM providers response format
                    async with self.session.get(
                        f"{self.backend_url}/api/v1/llm/providers", headers=headers
                    ) as llm_response:
        ...
        try:
            ...
            for scenario in error_scenarios:
                ...
                if actual_status == scenario["expected_status"]:
                    consistent_errors += 1
                    print(
                        f"✅ {scenario['name']}: Correct error status {actual_status}"
                    )
            ...
            if consistent_errors == len(error_scenarios):
                ...
                print(
                    f"⚠️ {consistent_errors}/{len(error_scenarios)} error scenarios consistent"
                )
                return False
        ...
        try:
            ...
            for test in performance_tests:
                ...
                try:
                    if test["method"] == "GET":
                        async with self.session.get(
                            f"{self.backend_url}{test['url']}", headers=test["headers"]
                        ) as response:
                    ...
                    performance_results.append(
                        {
                            ...
                            "status": status,
                            "passed": response_time_ms <= test["threshold_ms"]
                            and status == 200,
                        }
                    )
                    ...
                    if response_time_ms <= test["threshold_ms"] and status == 200:
                        print(
                            f"✅ {test['name']}: {response_time_ms:.1f}ms (< {test['threshold_ms']}ms)"
                        )
                    else:
                        print(
                            f"❌ {test['name']}: {response_time_ms:.1f}ms (> {test['threshold_ms']}ms) or status {status}"
                        )
    ...
    async def test_data_consistency(self):
        ...
        try:
            ...
            # Check consistency
            if user_me_data["email"] == user_verify_data.get("email") and user_me_data[
                "user_id"
            ] == user_verify_data.get("user_id"):
            ...
            async with self.session.get(
                f"{self.backend_url}/api/v1/vector-search/collections", headers=headers
            ) as response:
            ...
            consistency_check = all(
                col in available_collection_types for col in searched_collections
            )
