...
class TestDataSourcesAPI:
    ...
    def test_discover_sources_with_filter(self, client, temp_bootstrap_dir):
        ...
        response = client.get(
            f"/api/v1/data-sources/discover?bootstrap_dir={temp_bootstrap_dir}&category=academic"
        )
        assert response.status_code == 200
    ...
    def test_get_bootstrap_stats(self, client, temp_bootstrap_dir):
        ...
        response = client.get(
            f"/api/v1/data-sources/bootstrap/stats?bootstrap_dir={temp_bootstrap_dir}"
        )
        assert response.status_code == 200
    ...
    def test_get_bootstrap_stats_nonexistent_dir(self, client):
        ...
        response = client.get(
            "/api/v1/data-sources/bootstrap/stats?bootstrap_dir=/nonexistent/path"
        )
...
class TestDataSourcesIntegration:
    ...
    def test_integration_with_real_bootstrap_data(self, client):
        ...
        response = client.get(
            f"/api/v1/data-sources/bootstrap/stats?bootstrap_dir={bootstrap_path}"
        )
        assert response.status_code == 200
