...
class APMTracker:
    ...
    def end_transaction(self, transaction_id, status="success"):
        if transaction_id in self.active_transactions:
            ...
            result = {"duration_ms": duration, "status": status, "name": tx["name"]}
    ...
    def get_performance_summary(self):
        ...
        successful = [
            t for t in self.completed_transactions if t["status"] == "success"
        ]
        avg_duration = sum(t["duration_ms"] for t in self.completed_transactions) / len(
            self.completed_transactions
        )
    ...
    def get_slow_transactions(self, threshold_ms=1000):
        return [
            t for t in self.completed_transactions if t["duration_ms"] > threshold_ms
        ]
...
class TransactionContext:
    ...
    def __enter__(self):
        self.transaction_id = self.tracker.start_transaction(
...
class TestAPMTracker:
    ...
    def test_start_transaction(self, apm_tracker):
        ...
        transaction_id = apm_tracker.start_transaction(
            name="test_transaction", transaction_type="request"
        )
...
class TestMonitoringIntegration:
    ...
    async def test_apm_integration(self, apm_tracker):
        ...
        error_id = apm_tracker.record_error(
            {"exception": "TestError", "message": "Integration test error"}
        )
