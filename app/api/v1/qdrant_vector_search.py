...
class DocumentSearchRequest(BaseModel):
    ...
    collection_types: Optional[List[str]] = Field(None, description="Collections to search")
...
async def search_documents(request: DocumentSearchRequest):
    ...
    try:
        ...
        if request.collection_types:
            try:
                collection_types = [CollectionType(ct.lower()) for ct in request.collection_types]
            except ValueError as e:
                raise HTTPException(status_code=400, detail=f"Invalid collection type: {e}")
