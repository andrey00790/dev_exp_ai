...
class TestSearchIntegration:
    ...
    @patch("domain.integration.search_service.SearchService")
...
class TestServiceIntegration:
    ...
    def test_ai_analytics_service_integration(self, mock_service):
        ...
        # Настройка mock
        mock_instance = Mock()
        mock_instance.analyze_usage = AsyncMock(
            return_value={
                "total_requests": 100,
    ...
    def test_input_validation_integration(self):
        ...
        try:
            request = AdvancedSearchRequest(
                query="test", search_type="semantic", limit=20
            )
        ...
        with pytest.raises(Exception):
            AdvancedSearchRequest(query="test", limit=1000)  # Превышает максимум
...
class TestErrorHandlingIntegration:
    ...
    def test_validation_error_handling(self, client):
        ...
        response = client.post(
            "/api/v1/search/advanced/", json={"invalid_field": "value"}
        )
