...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if request.url.path.startswith("/api/v1/auth") or request.url.path.startswith("/api/v1/budget"):
...
SECURITY_RATE_LIMITS = {
    ...
    "/auth/reset-password": {"calls": 3, "period": 300},  # 3 password resets per 5 minutes
}
...
class SecurityLogger:
    ...
    def log_authentication_attempt(self, ip: str, email: str, success: bool, reason: str = None):
