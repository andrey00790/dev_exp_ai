...
class InputValidator:
    ...
    def validate_and_sanitize_string(
        self, value: str, field_name: str = "field"
    ) -> str:
        ...
        if len(value) > MAX_STRING_LENGTH:
            raise ValueError(
                f"{field_name} exceeds maximum length of {MAX_STRING_LENGTH}"
            )
        ...
        for pattern in self.xss_patterns:
            if pattern.search(value):
                logger.warning(
                    f"XSS attempt detected in {field_name}: {value[:100]}..."
                )
                raise ValueError(f"Invalid content detected in {field_name}")
    ...
    def validate_number(
        self, value: Union[int, float, str], field_name: str = "number"
    ) -> Union[int, float]:
    ...
    def validate_decimal(
        self, value: Union[str, float, int], field_name: str = "decimal"
    ) -> Decimal:
    ...
    def validate_array(self, value: List[Any], field_name: str = "array") -> List[Any]:
        ...
        if len(value) > MAX_ARRAY_LENGTH:
            raise ValueError(
                f"{field_name} exceeds maximum length of {MAX_ARRAY_LENGTH}"
            )
    ...
    def validate_object(
        self, value: Dict[str, Any], field_name: str = "object", depth: int = 0
    ) -> Dict[str, Any]:
        ...
        for key, val in value.items():
            ...
            # Validate value
            sanitized_val = self.validate_any_value(
                val, f"{field_name}.{key}", depth + 1
            )
            sanitized_object[sanitized_key] = sanitized_val

        return sanitized_object

    def validate_any_value(
        self, value: Any, field_name: str = "value", depth: int = 0
    ) -> Any:
    ...
    def validate_file_upload(self, filename: str, content_type: str, size: int) -> bool:
        ...
        if size > MAX_REQUEST_SIZE:
            raise ValueError(
                f"File size exceeds maximum allowed size of {MAX_REQUEST_SIZE} bytes"
            )
    ...
    def validate_rfc_generation_request(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Validate title
        validated_data["title"] = self.validate_and_sanitize_string(
            data["title"], "title"
        )
        ...
        validated_data["description"] = self.validate_and_sanitize_string(
        ...
        if "category" in data:
            validated_data["category"] = self.validate_and_sanitize_string(
                data["category"], "category"
            )

        if "priority" in data:
            validated_data["priority"] = self.validate_and_sanitize_string(
                data["priority"], "priority"
            )

        if "metadata" in data:
            validated_data["metadata"] = self.validate_object(
    ...
    try:
        ...
        if request.method in ["POST", "PUT", "PATCH"]:
            try:
                ...
                if body:
                    try:
                        ...
                        if request.url.path.endswith("/generate"):
                            validated_data = (
                                input_validator.validate_rfc_generation_request(
                                    json_data
                                )
                            )
                            ...
                            if "query" in json_data:
                                json_data["query"] = (
                                    input_validator.validate_search_query(
                                        json_data["query"]
                                    )
                                )
                ...
                logger.warning(
                    f"Input validation failed for {request.url.path}: {str(e)}"
                )
                return JSONResponse(
                    status_code=400, content={"detail": f"Invalid input: {str(e)}"}
                )

        # Validate query parameters
        for key, value in request.query_params.items():
            try:
                input_validator.validate_and_sanitize_string(
                    value, f"query_param_{key}"
                )
        ...
        return JSONResponse(
            status_code=500, content={"detail": "Internal validation error"}
        )
...
def validate_budget_input(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "user_id" in data:
        validated["user_id"] = input_validator.validate_and_sanitize_string(
            data["user_id"], "user_id"
        )

    if "new_budget_limit" in data:
        validated["new_budget_limit"] = float(
            input_validator.validate_decimal(
                data["new_budget_limit"], "new_budget_limit"
            )
        )

    if "reason" in data:
        validated["reason"] = input_validator.validate_and_sanitize_string(
            data["reason"], "reason"
        )
