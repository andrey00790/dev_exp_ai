...
class TestVectorSearchService:
    ...
    async def test_search_basic(self, vector_search_service):
        ...
        vector_search_service.embedding_service.embed_text.assert_called_once_with(query)
...
class TestHIPAAComplianceService:
    ...
    def test_detect_phi_ssn(self, hipaa_service):
        ...
        # Should detect SSN
        ssn_detection = next((item for item in phi_detected if item["type"] == "SSN"), None)
    ...
    def test_detect_phi_phone(self, hipaa_service):
        ...
        phone_detection = next((item for item in phi_detected if item["type"] == "PHONE"), None)
    ...
    def test_detect_phi_email(self, hipaa_service):
        ...
        email_detection = next((item for item in phi_detected if item["type"] == "EMAIL"), None)
