...
ПОДКЛЮЧЕН К ОБУЧЕНИЮ: отправляет фидбек для переобучения модели
...
class RealAPISemanticSearchEvaluator:
    ...
    async def send_evaluation_feedback(self, query: str, search_results: Dict, 
        ...
        """Отправляет фидбек по результатам evaluation для обучения модели"""
        try:
            ...
            feedback_payload = {
                ...
                "feedback_type": "like" if precision_score >= 0.3 else "dislike",
                "rating": rating,
                "comment": f"Eval: P@3={precision_score:.3f}, Lang={language}, Query='{query[:50]}...'",
    ...
    async def evaluate_single_query(self, query: str, expected_docs: List[str], 
        ...
        # 🧠 КРИТИЧНО: Отправляем фидбек по результатам evaluation для обучения
    ...
    async def evaluate_role_queries(self, role_name: str, role_data: List[Dict]) -> List[EvaluationResult]:
        ...
        for query_data in role_data:
            ...
            feedback_icon = "✅" if result.feedback_sent else "❌"
            self.logger.info(f"[{language.upper()}] '{query[:50]}...' | P@1: {result.precision_at_1:.3f} | "
                           f"P@3: {result.precision_at_3:.3f} | MRR: {result.mrr:.3f} | Feedback: {feedback_icon}")
    ...
    def log_detailed_results(self, metrics: AggregateMetrics):
        ...
        self.logger.info(f"⏱️ Avg Execution Time: {metrics.avg_execution_time_ms:.2f}ms")
    ...
    async def trigger_retraining_if_needed(self, metrics: AggregateMetrics) -> Dict[str, Any]:
        """Запускает переобучение если качество низкое"""
        try:
            needs_retraining = (
                metrics.avg_precision_at_3 < 0.3 or 
                metrics.avg_mrr < 0.2 or
                metrics.feedback_success_rate < 0.8
            )
            
            if needs_retraining:
                self.logger.info("🧠 Quality below threshold, triggering retraining...")
                
                retraining_payload = {
                    "context": "search_result",
                    "trigger_reason": "evaluation_quality_low",
                    "min_examples": 5
                }
                
                response = await self.client.post(f"{self.api_base_url}/api/v1/feedback/retrain", 
                                                json=retraining_payload)
                
                if response.status_code == 200:
                    result = response.json()
                    job_id = result.get('job_id', 'unknown')
                    self.logger.info(f"✅ Retraining job started: {job_id}")
                    return {"triggered": True, "job_id": job_id, "reason": "quality_low"}
                else:
                    self.logger.warning(f"❌ Retraining request failed: {response.status_code}")
                    return {"triggered": False, "error": f"HTTP {response.status_code}"}
            else:
                self.logger.info("✅ Quality meets threshold, no retraining needed")
                return {"triggered": False, "reason": "quality_acceptable"}
    ...
    def save_results_to_file(self, metrics: AggregateMetrics, output_file: str):
        results_data = {
            ...
            "evaluation_type": "real_api_with_feedback",
...
async def main():
    parser = argparse.ArgumentParser(description="Evaluate multilingual semantic search with REAL API + Learning")
    ...
    parser.add_argument("--trigger-retraining", action="store_true", help="Auto-trigger retraining if quality low")
    ...
    try:
        ...
        logger.info(f"🚀 Starting REAL API evaluation with FEEDBACK LEARNING")
        ...
        # Quality assessment
        precision_threshold = 0.3  # Реалистичный порог для начала
        mrr_threshold = 0.2
        ...
        if args.trigger_retraining:
            retraining_result = await evaluator.trigger_retraining_if_needed(metrics)
            if retraining_result["triggered"]:
                logger.info(f"🔄 Model retraining initiated: {retraining_result}")
        
        logger.info("🎉 Real API evaluation with feedback learning completed!")
