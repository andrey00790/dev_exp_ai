...
from src.scripts.user_config_manager import (UserConfigManager,
        ...
        for pid in project_ids:
            ...
            def _fetch_project(p: int) -> Optional[Dict[str, Any]]:
                try:
                    ...
                    readme = project.files.get(
                        file_path="README.md", ref=project.default_branch or "main"
                    )
...
class DataSyncService:
    ...
    async def sync_source(
        self, user_id: int, source_type: str, source_name: str
    ) -> None:
        """Синхронизирует один источник для пользователя."""
        logger.info(
            f"Запуск синхронизации для: user_id={user_id}, source={source_type}:{source_name}"
        )
        ...
        try:
            ...
            source_config = next(
                (s for s in sources if s.source_name == source_name), None
            )

            if not source_config or not source_config.is_enabled_semantic_search:
                logger.warning(
                    f"Источник {source_type}:{source_name} не найден или отключен."
                )
                return

            # Обновляем статус
            self.config_manager.update_sync_status(
                user_id, source_type, source_name, "running"
            )
            ...
            logger.info(
                f"Получено {len(documents)} документов из {source_type}:{source_name}. Начинаем индексацию..."
            )

            # Обновляем статус и время последней синхронизации
            self.config_manager.update_sync_status(
                user_id, source_type, source_name, "success"
            )
            logger.info(
                f"Синхронизация для {source_type}:{source_name} успешно завершена."
            )

        except Exception as e:
            logger.error(
                f"Ошибка при синхронизации {source_type}:{source_name}: {e}",
                exc_info=True,
            )
            self.config_manager.update_sync_status(
                user_id, source_type, source_name, "error", str(e)
            )
    ...
    def schedule_sync_jobs(self):
        ...
        users = (
            self.config_manager.get_all_users()
        )  # Предполагаем, что есть такой метод
        ...
        for user_id in users_for_startup_sync:
            ...
            for source in sources:
                if source.auto_sync_on_startup and source.is_enabled_semantic_search:
                    asyncio.create_task(
                        self.sync_source(
                            user_id, source.source_type, source.source_name
                        )
                    )
