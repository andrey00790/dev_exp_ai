    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "easy_implementation": len([i for i in insights if i.implementation_effort == "easy"])
            },
            "analysis_timestamp": datetime.utcnow().isoformat()
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Cost insights analysis failed: {str(e)}")
...
async def run_batch_analysis(
    background_tasks: BackgroundTasks,
    time_range_days: int = Query(30, description="Time range for analysis", ge=1, le=90),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        async def run_analysis():
            ...
            for metric_type in [MetricType.LATENCY, MetricType.ACCURACY, MetricType.COST]:
