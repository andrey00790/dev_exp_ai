...
class LiveAIEvaluator:
    ...
    async def test_ai_search_with_feedback(
        self, query: str, language: str = "ru"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            search_payload = {"query": query, "limit": 5, "language": language}
                ...
                try:
                    if method == "POST":
                        response = await self.client.post(
                            f"{self.base_url}{endpoint}", json=search_payload
                        )
                    else:
                        response = await self.client.get(
                            f"{self.base_url}{endpoint}", params=search_payload
                        )
            ...
            results_count = len(
                search_result.get("results", search_result.get("documents", []))
            )
            ...
            # 3. Отправляем фидбек
            feedback_success = await self.send_feedback(
                query, search_result, quality_score
            )
    ...
    def _evaluate_search_quality(self, search_result: Dict, query: str) -> float:
        ...
        results = search_result.get(
            "results", search_result.get("documents", search_result.get("data", []))
        )
    ...
    async def send_feedback(
        self, query: str, search_result: Dict, quality_score: float
    ) -> bool:
        ...
        try:
            ...
            for endpoint in feedback_endpoints:
                try:
                    response = await self.client.post(
                        f"{self.base_url}{endpoint}", json=feedback_payload
                    )
                    if response.status_code == 200:
    ...
    async def test_feedback_driven_improvements(self) -> Dict[str, Any]:
        ...
        for test_case in test_queries:
            result = await self.test_ai_search_with_feedback(
                test_case["query"], test_case["language"]
            )
        ...
        avg_quality = sum(
            r.get("quality_score", 0) for r in results if r["success"]
        ...
        return {
            ...
            "recommendation": (
                ...
                if avg_quality < 60
                else "Model performance acceptable"
            ),
        }
