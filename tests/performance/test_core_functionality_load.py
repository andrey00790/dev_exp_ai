...
class CoreFunctionalityLoadTester:
    ...
    async def run_health_check_load_test(
        self, concurrent_users: int = 50, requests_per_user: int = 20
    ) -> LoadTestMetrics:
        """Test health check endpoint under load"""
        logger.info(
            f"🏃‍♂️ Running health check load test: {concurrent_users} users, {requests_per_user} req/user"
        )
        ...
        logger.info(
            f"✅ Health check load test completed: {successful_requests}/{total_requests} success, "
    ...
    async def run_authentication_load_test(
        self, concurrent_users: int = 30, requests_per_user: int = 5
    ) -> LoadTestMetrics:
        """Test authentication endpoints under load"""
        logger.info(
            f"🔐 Running authentication load test: {concurrent_users} users, {requests_per_user} req/user"
        )
        ...
        self.test_results.append(metrics)
        logger.info(
            f"✅ Authentication load test completed: {successful_requests}/{total_requests} success"
        )

        return metrics

    async def run_websocket_load_test(
        self, concurrent_connections: int = 25, messages_per_connection: int = 10
    ) -> LoadTestMetrics:
        """Test WebSocket endpoints under load"""
        logger.info(
            f"🔌 Running WebSocket load test: {concurrent_connections} connections, {messages_per_connection} msgs/conn"
        )
        ...
        self.test_results.append(metrics)
        logger.info(
            f"✅ WebSocket load test completed: {successful_requests}/{total_requests} success"
        )

        return metrics

    async def run_monitoring_load_test(
        self, concurrent_users: int = 20, requests_per_user: int = 15
    ) -> LoadTestMetrics:
        """Test monitoring endpoints under load"""
        logger.info(
            f"📊 Running monitoring load test: {concurrent_users} users, {requests_per_user} req/user"
        )
            ...
            async with httpx.AsyncClient(app=None, base_url=self.base_url) as client:
                ...
                for i in range(requests_per_user):
                    ...
                    try:
                        ...
                        if i % 4 == 0:
                            response = await client.get(
                                "/api/v1/monitoring/metrics/current", headers=headers
                            )
                        elif i % 4 == 1:
                            response = await client.get(
                                "/api/v1/monitoring/metrics/history", headers=headers
                            )
                        elif i % 4 == 2:
                            response = await client.get(
                            ...
                            response = await client.get(
                                "/api/v1/realtime-monitoring/health", headers=headers
                            )
        ...
        self.test_results.append(metrics)
        logger.info(
            f"✅ Monitoring load test completed: {successful_requests}/{total_requests} success"
        )

        return metrics

    async def run_optimization_load_test(
        self, concurrent_users: int = 15, requests_per_user: int = 8
    ) -> LoadTestMetrics:
        """Test optimization endpoints under load"""
        logger.info(
            f"⚡ Running optimization load test: {concurrent_users} users, {requests_per_user} req/user"
        )
                ...
                for i in range(requests_per_user):
                    ...
                    try:
                        ...
                        if i % 3 == 0:
                            ...
                            # History request
                            response = await client.get(
                                "/api/v1/optimization/history", headers=headers
                            )
        ...
        self.test_results.append(metrics)
        logger.info(
            f"✅ Optimization load test completed: {successful_requests}/{total_requests} success"
        )
    ...
    async def run_comprehensive_load_test(self) -> Dict[str, Any]:
        ...
        # Run all load tests
        health_metrics = await self.run_health_check_load_test(
            concurrent_users=100, requests_per_user=30
        )
        auth_metrics = await self.run_authentication_load_test(
            concurrent_users=50, requests_per_user=10
        )
        ws_metrics = await self.run_websocket_load_test(
            concurrent_connections=40, messages_per_connection=15
        )
        monitor_metrics = await self.run_monitoring_load_test(
            concurrent_users=30, requests_per_user=20
        )
        opt_metrics = await self.run_optimization_load_test(
            concurrent_users=25, requests_per_user=12
        )
        ...
        logger.info(
            f"✅ Comprehensive load test completed: {overall_success_rate:.1f}% success rate, {overall_rps:.1f} RPS"
        )
    ...
    def _analyze_performance(self, metrics: List[LoadTestMetrics]) -> Dict[str, Any]:
        ...
        analysis = {
            "best_performing": {
                "by_response_time": min(
                    metrics, key=lambda m: m.avg_response_time
                ).test_name,
                "by_throughput": max(
                    metrics, key=lambda m: m.requests_per_second
                ).test_name,
                "by_success_rate": max(
                    metrics,
                    key=lambda m: (
        ...
        for metric in metrics:
            if metric.error_rate > 5.0:
                analysis["performance_concerns"].append(
                    f"{metric.test_name}: High error rate ({metric.error_rate:.1f}%)"
                )

            if metric.p95_response_time > 1.0:
                analysis["performance_concerns"].append(
                    f"{metric.test_name}: Slow P95 response time ({metric.p95_response_time:.3f}s)"
                )

            if metric.requests_per_second < 10:
                analysis["performance_concerns"].append(
                    f"{metric.test_name}: Low throughput ({metric.requests_per_second:.1f} RPS)"
                )

        # Generate recommendations
        if not analysis["performance_concerns"]:
            analysis["recommendations"].append(
                "System performance is excellent across all test scenarios"
            )
        else:
...
class TestCoreFunctionalityLoad:
    ...
    async def test_health_check_load(self):
        ...
        metrics = await tester.run_health_check_load_test(
            concurrent_users=25, requests_per_user=10
        )

        # Performance assertions
        assert (
            metrics.error_rate < 10.0
        ), f"Error rate too high: {metrics.error_rate:.1f}%"
        assert (
            metrics.avg_response_time < 0.5
        ), f"Average response time too slow: {metrics.avg_response_time:.3f}s"
        assert (
            metrics.requests_per_second > 5
        ), f"Throughput too low: {metrics.requests_per_second:.1f} RPS"

    async def test_authentication_load(self):
        """Test authentication under moderate load"""
        tester = CoreFunctionalityLoadTester()
        metrics = await tester.run_authentication_load_test(
    ...
    async def test_websocket_load(self):
        ...
        tester = CoreFunctionalityLoadTester()
        metrics = await tester.run_websocket_load_test(
            concurrent_connections=15, messages_per_connection=8
        )
        ...
        assert (
            metrics.avg_response_time < 0.8
        ), f"Average response time too slow: {metrics.avg_response_time:.3f}s"

    async def test_monitoring_load(self):
        """Test monitoring endpoints under moderate load"""
        tester = CoreFunctionalityLoadTester()
        metrics = await tester.run_monitoring_load_test(
            concurrent_users=15, requests_per_user=10
        )
    ...
    async def test_optimization_load(self):
        ...
        tester = CoreFunctionalityLoadTester()
        metrics = await tester.run_optimization_load_test(
            concurrent_users=10, requests_per_user=6
        )

        # Performance assertions (more lenient for optimization)
        assert (
            metrics.error_rate < 30.0
        ), f"Error rate too high: {metrics.error_rate:.1f}%"
        assert (
...
class TestHighLoadScenarios:
    ...
    async def test_stress_health_check(self):
        ...
        assert (
            metrics.error_rate < 20.0
        ), f"Stress test error rate too high: {metrics.error_rate:.1f}%"
        assert (
            metrics.requests_per_second > 50
        ), f"Stress test throughput too low: {metrics.requests_per_second:.1f} RPS"
    ...
    async def test_comprehensive_load(self):
        ...
        assert (
            results["test_summary"]["overall_success_rate"] > 70.0
