    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if result:
                    # PostgreSQL JSONB возвращается как Python объект
...
class TestPostgreSQLCache:
    ...
    def test_cache_expiration(self, cache_client):
        ...
        # Устанавливаем ключ с TTL 2 секунды
        assert cache_client.set('expiring_key', 'expiring_value', ttl_seconds=2) is True
        
        # Проверяем что ключ существует
        assert cache_client.get('expiring_key') == 'expiring_value'
        assert cache_client.exists('expiring_key') is True
        
        # Ждем истечения TTL
        time.sleep(3)
        
        # Проверяем что ключ исчез
        assert cache_client.get('expiring_key') is None
        assert cache_client.exists('expiring_key') is False
    ...
    def test_cache_cleanup_expired(self, cache_client):
        ...
        # Проверяем результат
        assert cache_client.exists('key1') is False
        assert cache_client.exists('key2') is True
        assert cache_client.exists('key3') is True
    ...
    def test_cache_performance(self, cache_client):
        ...
        assert write_time < 5.0, f"Запись 100 ключей заняла {write_time:.2f}s (ожидалось < 5s)"
        assert read_time < 2.0, f"Чтение 100 ключей заняло {read_time:.2f}s (ожидалось < 2s)"
    ...
    def test_cleanup_expired_sessions(self, db_connection):
        ...
        with db_connection.cursor() as cursor:
            ...
            cursor.execute("""
                INSERT INTO cache_data.user_sessions (session_id, user_id, session_data, expires_at)
                VALUES (%s, %s, %s, %s)
            """, ('active_session', 1, '{"status": "active"}', current_time + timedelta(hours=1)))
            
            # Истекшая сессия
            cursor.execute("""
                INSERT INTO cache_data.user_sessions (session_id, user_id, session_data, expires_at)
                VALUES (%s, %s, %s, %s)
            """, ('expired_session', 2, '{"status": "expired"}', current_time - timedelta(hours=1)))
