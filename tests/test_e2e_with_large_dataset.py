...
class E2EDataManager:
    ...
    def generate_confluence_data(self, count: int = 50):
        ...
        topics = [
            "OAuth 2.0 Implementation", "JWT Token Management", "API Security",
            ...
            "Container Orchestration", "CI/CD Pipelines", "Monitoring and Logging",
            "Performance Optimization", "Error Handling", "Rate Limiting",
            "Authentication Patterns", "Authorization Models", "Data Encryption",
            "Backup and Recovery", "Disaster Recovery", "High Availability",
            "Scalability Patterns", "Network Security", "Application Security",
            "Code Review Process", "Testing Strategies", "Deployment Automation",
            "Infrastructure as Code", "Cloud Migration", "Cost Optimization",
            "Compliance and Governance", "Documentation Standards", "API Versioning",
            "Event Sourcing", "CQRS Pattern", "Saga Pattern",
            "Circuit Breaker Pattern", "Bulkhead Pattern", "Retry Patterns",
            "Health Check Implementation", "Service Mesh", "API Gateway Design",
            "Data Synchronization", "Eventual Consistency", "Distributed Tracing",
            "Metrics Collection", "Alerting Strategies", "Incident Response",
            "Root Cause Analysis", "Capacity Planning", "Resource Management"
        ]
...
"""
            ...
            filename = TEST_DATA_DIR / "confluence" / f"{topic.lower().replace(' ', '_')}_guide.md"
    ...
    def generate_jira_data(self, count: int = 200):
        ...
        for i in range(count):
            ...
            issue = {
                ...
                "story_points": [1, 2, 3, 5, 8, 13][i % 6] if issue_type == "Story" else None,
    ...
    def _generate_labels(self, issue_type: str) -> List[str]:
        ...
        common_labels = ["backend", "frontend", "api", "security", "performance", "testing", "documentation"]
        type_labels = {
        ...
        return type_labels[issue_type] + [common_labels[hash(issue_type) % len(common_labels)]]
    ...
    def generate_gitlab_data(self, count: int = 10):
        ...
        repo_names = [
            ...
            "auth-service", "analytics-service", "reporting-service", "admin-portal",
            "mobile-app-backend", "data-processing-service"
        ]
...
"""
    ...
    def generate_training_dataset(self):
        ...
        training_dataset = {
            "metadata": {
                ...
                "languages": ["ru", "en"],
                "categories": ["authentication", "architecture", "security", "performance"],
                "data_source": "auto_generated_e2e_dataset",
                "config_loaded": "dataset_config.yml" if Path("dataset_config.yml").exists() else "default"
            },
            "training_pairs": training_pairs,
            "evaluation_queries": [
                {"text": "OAuth authentication", "language": "en", "expected_category": "authentication"},
                {"text": "микросервисы", "language": "ru", "expected_category": "architecture"},
                {"text": "API security", "language": "en", "expected_category": "security"},
                {"text": "оптимизация производительности", "language": "ru", "expected_category": "performance"}
            ]
        }
        
        with open(TEST_DATA_DIR / "dataset" / "training_dataset.json", 'w', encoding='utf-8') as f:
    ...
    def train_model_if_needed(self):
        ...
        try:
            ...
            print(f"🧠 Training model with {len(training_data['training_pairs'])} training pairs...")
...
class TestE2ELargeDataset:
    ...
    def test_confluence_data_generated(self, large_dataset_environment):
        ...
        assert len(md_files) >= 10, f"Expected at least 10 Confluence files, got {len(md_files)}"
    ...
    def test_jira_data_generated(self, large_dataset_environment):
        ...
        assert len(jira_data["issues"]) >= 50, f"Expected at least 50 issues, got {len(jira_data['issues'])}"
    ...
    def test_gitlab_data_generated(self, large_dataset_environment):
        ...
        assert len(repo_dirs) >= 5, f"Expected at least 5 repositories, got {len(repo_dirs)}"
    ...
    def test_model_training_integration(self, large_dataset_environment):
        ...
        if training_file.exists():
            ...
            languages = set(pair["language"] for pair in training_data["training_pairs"])
    ...
    def test_data_consistency(self, large_dataset_environment):
        ...
        assert confluence_files >= 10, f"Insufficient Confluence files: {confluence_files}"
        ...
        gitlab_repos = len([d for d in (TEST_DATA_DIR / "gitlab").iterdir() if d.is_dir()])
...
if __name__ == "__main__":
    ...
    print(f"  - Confluence: {len(list((TEST_DATA_DIR / 'confluence').glob('*.md')))} documents")
    print(f"  - Jira: Large issue dataset")
    print(f"  - GitLab: {len([d for d in (TEST_DATA_DIR / 'gitlab').iterdir() if d.is_dir()])} repositories")
