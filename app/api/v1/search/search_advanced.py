...
async def get_filter_suggestions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get filter suggestions: {str(e)}"
        )
...
async def get_filter_presets(
    ...
    presets = {
        ...
        "high_quality": {
            ...
            "filters": {"quality": {"min_quality_score": 0.8}},
        },
    }

    return {"presets": presets, "total_presets": len(presets)}
...
async def export_search_results(
    ...
    if format not in ["json", "csv", "xlsx"]:
        raise HTTPException(
            status_code=400, detail="Unsupported export format. Use: json, csv, xlsx"
        )
    ...
    if request.time:
        if request.time.updated_after and request.time.updated_before:
            filter_obj.by_date_range(
                request.time.updated_after, request.time.updated_before, "updated_at"
            )
...
def _calculate_quality_stats(results: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    relevance_scores = [
        r.get("status_info", {}).get("relevance_score", 0) for r in results
    ]
