...
class TestUsersAPIFinal:
    ...
    def test_update_user_settings(self, authenticated_client):
        ...
        response = authenticated_client.put("/api/v1/users/current/settings", json=settings_data)
...
class TestUsersServiceIntegrationFinal:
    ...
    def test_data_sources_defaults(self, authenticated_client):
        ...
        if response.status_code == 200:
            ...
            jira_source = next((ds for ds in data_sources if ds["source_type"] == "jira"), None)
            if jira_source:
                assert jira_source["is_enabled_semantic_search"] == True
                assert jira_source["is_enabled_architecture_generation"] == True
            
            user_files_source = next((ds for ds in data_sources if ds["source_type"] == "user_files"), None)
...
class TestUsersAPIErrorHandlingFinal:
    ...
    def test_database_error_handling(self):
        ...
        mock_manager = Mock()
        mock_manager.create_user_with_defaults.side_effect = Exception("Database connection failed")
        ...
        user_data = {
            "username": "test_user_unique_4",
            "email": "test4@example.com"
        }
        
        response = client.post("/api/v1/users", json=user_data)
        
        # Должна быть ошибка сервера, так как mock выбрасывает исключение
...
class TestUsersAPIValidationFinal:
    ...
    def test_update_settings_invalid_data(self, authenticated_client):
        ...
        response = authenticated_client.put("/api/v1/users/current/settings", json=invalid_settings)
        assert response.status_code in [422, 400, 404]
