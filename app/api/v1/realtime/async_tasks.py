...
from app.performance.async_processor import (TaskPriority, TaskStatus,
                                             async_processor)
...
class TaskSubmissionRequest(BaseModel):
    ...
    priority: str = Field(
        default="normal", description="Task priority: low, normal, high, urgent"
    )
    notification_callback: Optional[str] = Field(
        None, description="WebSocket callback for notifications"
    )
...
async def submit_async_task(
    task_request: TaskSubmissionRequest, current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"✅ Task submitted by {current_user.email}: {result['task_id']} ({task_request.task_func})"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Task submission failed after retries: {str(e)}"
        )
...
async def get_task_status(
    ...
    try:
        ...
        logger.info(
            f"✅ Task status retrieved: {task_id} for user {current_user.user_id}"
        )
        ...
        raise HTTPException(
            status_code=504, detail="Task status lookup timed out: System overloaded"
        )
    except AsyncRetryError as e:
        logger.error(f"❌ Task status lookup failed after retries: {e}")
        raise HTTPException(
            status_code=500, detail=f"Status lookup failed after retries: {str(e)}"
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def cancel_task(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to cancel task"
            )
...
async def get_user_tasks(
    ...
    try:
        ...
        # Get user tasks
        user_tasks = await async_processor.get_user_tasks(
            current_user.user_id, status_enum
        )
            ...
            response_data.update(
                {
                    ...
                    "queue_details": {
                        ...
                        "notification_queue_size": stats.get(
...
async def cleanup_completed_tasks(
    ...
    try:
        ...
        logger.info(
            f"Task cleanup performed by admin {current_user.email}: {cleaned_count} tasks removed"
        )
...
async def _submit_task_internal(
    task_request: TaskSubmissionRequest, current_user: User
) -> Dict[str, Any]:
