...
async def ingest_metric(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Metric ingestion failed: {str(e)}")
...
async def acknowledge_alert(
    ...
    try:
        await realtime_monitoring_service.acknowledge_alert(
            alert_id=alert_id,
            acknowledged_by=request.user_id or str(current_user.id)
        )
...
async def resolve_alert(
    ...
    try:
        await realtime_monitoring_service.resolve_alert(
            alert_id=alert_id,
            resolved_by=request.user_id or str(current_user.id)
        )
...
async def get_anomalies(
    ...
    try:
        ...
        for anomaly in filtered_anomalies:
            ...
            summary["by_metric"][anomaly.metric] = summary["by_metric"].get(anomaly.metric, 0) + 1
...
async def get_sla_status(
    ...
    try:
        ...
        for sla_id, status in sla_status.items():
            ...
            if status["is_active"]:
                overall_compliance = min(overall_compliance, status["current_compliance"])
...
async def get_live_metrics(
    ...
    try:
        ...
        # Anomaly summary
        recent_anomalies = realtime_monitoring_service.get_recent_anomalies(1)  # Last hour
        anomalies_summary = {
            "total": len(recent_anomalies),
            "critical": len([a for a in recent_anomalies if a.severity == AlertSeverity.CRITICAL]),
            "high": len([a for a in recent_anomalies if a.severity == AlertSeverity.HIGH]),
            "medium": len([a for a in recent_anomalies if a.severity == AlertSeverity.MEDIUM])
        }
...
async def get_dashboard_stats(
    ...
    try:
        ...
        stats.update({
            ...
            "data_ingestion_rate": len(realtime_monitoring_service.metric_buffer) * 10,  # Estimated
...
async def monitoring_health():
    ...
    try:
        ...
        return {
            ...
            "uptime_seconds": (datetime.utcnow() - realtime_monitoring_service.start_time).total_seconds(),
        ...
        raise HTTPException(status_code=503, detail=f"Monitoring service unhealthy: {str(e)}")
...
async def get_performance_summary(
    ...
    try:
        ...
        performance_summary = {
            "total_transactions": 1000 + len(realtime_monitoring_service.metric_buffer) * 10,
            ...
            "slow_transactions_count": len([a for a in recent_anomalies 
                                          if "response_time" in a.metric.value]),
            "error_breakdown": {
                "timeout_errors": 5,
                "connection_errors": 3,
            ...
            "top_slow_endpoints": [
                ...
                {"endpoint": "/api/v1/analytics", "avg_duration_ms": 180.0, "count": 200}
            ],
            "alerts_summary": {
                "performance_alerts": len([a for a in active_alerts 
                                         if "performance" in a.title.lower() or 
                                            "response" in a.title.lower()]),
                "availability_alerts": len([a for a in active_alerts 
...
async def get_current_metrics_compat(
    current_user: User = Depends(get_current_user)
):
