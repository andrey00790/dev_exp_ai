...
class LoadTester:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if user_response.status_code == 200:
                    ...
                    logger.error(
                        f"❌ User authentication failed: {user_response.status_code}"
                    )
                    return False
    ...
    async def run_api_load_test(
        ...
        """
        logger.info(
            f"🚀 Starting API load test: {endpoint} ({concurrent_users} users, {requests_per_user} req/user)"
        )
        ...
        logger.info(
            f"✅ API load test completed: {successful_requests}/{total_requests} success, "
    ...
    async def run_websocket_load_test(
        ...
        """
        logger.info(
            f"🔌 Starting WebSocket load test: {concurrent_connections} connections"
        )
                ...
                async with websockets.connect(ws_url, timeout=10) as websocket:
                    ...
                    for i in range(messages_per_connection):
                        ...
                        await websocket.send(
                            json.dumps({"type": "ping", "message_id": i})
                        )
                        messages_sent += 1
        ...
        logger.info(
            f"✅ WebSocket load test completed: {successful_connections}/{total_connections} connections, "
    ...
    async def run_async_task_load_test(
        self, concurrent_submissions: int = 25, tasks_per_user: int = 3
    ) -> LoadTestResult:
        """
        ...
        """
        logger.info(
            f"⚡ Starting async task load test: {concurrent_submissions} concurrent submissions"
        )
        ...
        logger.info(
            f"✅ Async task load test completed: {successful_requests}/{total_requests} submissions, "
    ...
    def check_performance_regression(self, result: LoadTestResult) -> Dict[str, Any]:
        ...
        if not endpoint_key or endpoint_key not in self.performance_baselines:
            return {
                "status": "no_baseline",
                "message": "No baseline available for this test",
            }
        ...
        if result.avg_response_time > baseline.max_response_time:
            issues.append(
                f"Response time regression: {result.avg_response_time:.3f}s > {baseline.max_response_time:.3f}s"
            )
        ...
        if success_rate < baseline.min_success_rate:
            issues.append(
                f"Success rate regression: {success_rate:.1f}% < {baseline.min_success_rate:.1f}%"
            )

        # Check memory usage
        if result.memory_delta > baseline.max_memory_usage:
            issues.append(
                f"Memory usage regression: {result.memory_delta:.1f}MB > {baseline.max_memory_usage:.1f}MB"
            )

        # Check error rate
        if result.error_rate > baseline.max_error_rate:
            issues.append(
                f"Error rate regression: {result.error_rate:.1f}% > {baseline.max_error_rate:.1f}%"
            )
    ...
    async def run_comprehensive_load_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_result = await self.run_api_load_test(
                "/health", concurrent_users=100, requests_per_user=5
            )
            comprehensive_results["tests"]["health_check"] = health_result.__dict__
            comprehensive_results["regressions"]["health_check"] = (
                self.check_performance_regression(health_result)
            )

            # Test 2: Budget status load test (with caching)
            logger.info("💰 Testing budget status endpoint...")
            budget_result = await self.run_api_load_test(
                "/api/v1/budget/status", concurrent_users=50, requests_per_user=10
            )
            comprehensive_results["tests"]["budget_status"] = budget_result.__dict__
            comprehensive_results["regressions"]["budget_status"] = (
                self.check_performance_regression(budget_result)
            )

            # Test 3: Performance stats load test (admin endpoint)
            logger.info("📊 Testing performance stats endpoint...")
            perf_result = await self.run_api_load_test(
            ...
            comprehensive_results["regressions"]["performance_stats"] = (
                self.check_performance_regression(perf_result)
            )

            # Test 4: Async task submission load test
            logger.info("⚡ Testing async task submission...")
            task_result = await self.run_async_task_load_test(
                concurrent_submissions=30, tasks_per_user=2
            )
            comprehensive_results["tests"]["async_tasks"] = task_result.__dict__
            comprehensive_results["regressions"]["async_tasks"] = (
                self.check_performance_regression(task_result)
            )

            # Test 5: WebSocket connection load test
            logger.info("🔌 Testing WebSocket connections...")
            ws_result = await self.run_websocket_load_test(
            ...
            comprehensive_results["performance_summary"] = {
                ...
                "max_requests_per_second": max(
                    r.requests_per_second for r in all_results
                ),
            }

            # Check for any critical regressions
            critical_issues = []
            for test_name, regression_data in comprehensive_results[
                "regressions"
            ].items():
            ...
            comprehensive_results["overall_status"] = (
                "PASS" if not critical_issues else "FAIL"
            )

            logger.info(
                f"✅ Comprehensive load testing completed: {comprehensive_results['overall_status']}"
            )
    ...
    def generate_load_test_report(self, results: Dict[str, Any]) -> str:
        ...
        if "performance_summary" in results:
            ...
            report.append(
                f"   • Max Concurrent Users: {summary.get('max_concurrent_users', 0)}"
            )
            report.append(
                f"   • Max RPS: {summary.get('max_requests_per_second', 0):.1f}"
            )
        ...
        if "tests" in results:
            ...
            for test_name, test_data in results["tests"].items():
                ...
                report.append(
                    f"      • Concurrent Users: {test_data.get('concurrent_users', 0)}"
                )
                report.append(
                    f"      • Success Rate: {(test_data.get('successful_requests', 0) / max(test_data.get('total_requests', 1), 1) * 100):.1f}%"
                )
                report.append(
                    f"      • Avg Response: {test_data.get('avg_response_time', 0):.3f}s"
                )
                report.append(
                    f"      • P95 Response: {test_data.get('p95_response_time', 0):.3f}s"
                )
                report.append(
                    f"      • Requests/sec: {test_data.get('requests_per_second', 0):.1f}"
                )
                report.append(
                    f"      • Memory Delta: {test_data.get('memory_delta', 0):.1f}MB"
                )
...
class TestLoadTesting:
    ...
    def test_search_load_testing(self):
        ...
        def make_search_request(query_id):
            ...
            mock_response.json.return_value = {
                ...
                "results": [
                    {
                        "id": f"doc_{query_id}",
                        "title": f"Document {query_id}",
                        "score": 0.9,
                    }
                ],
                "total_results": 1,
                "search_time_ms": 50.0
                + (query_id % 10) * 10,  # Вариация времени поиска
            }
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(make_search_request, i) for i in range(num_searches)
            ]
            results = [
                future.result() for future in concurrent.futures.as_completed(futures)
            ]
        ...
        ), f"Only {success_count}/{num_searches} searches succeeded"
        assert (
            avg_response_time < 2.0
        ), f"Average response time too high: {avg_response_time}s"
        assert (
            p95_response_time < 3.0
        ), f"95th percentile response time too high: {p95_response_time}s"
        assert (
            avg_search_time < 200.0
    ...
    def test_memory_usage_simulation(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(simulate_memory_intensive_operation)
        ...
        assert (
            success_count == num_documents
        ), f"Only {success_count}/{num_documents} documents processed"
    ...
    def test_database_connection_pool_simulation(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            futures = [
                executor.submit(simulate_db_operation, i) for i in range(num_operations)
            ]
            results = [
                future.result() for future in concurrent.futures.as_completed(futures)
            ]
        ...
        ), f"Only {success_count}/{num_operations} DB operations succeeded"
        assert (
            unique_connections <= max_connections
        ), f"Too many connections created: {unique_connections}"
        assert (
            len(connection_pool) <= max_connections
        ), f"Connection pool exceeded limit: {len(connection_pool)}"
    ...
    def test_rate_limiting_simulation(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(make_request_with_rate_limit, i)
        ...
        assert (
            rate_limited_count >= expected_rate_limited
        ), f"Rate limiting not working: {rate_limited_count} limited"
        assert (
            success_count <= 5 * max_requests_per_user
        ), f"Too many successful requests: {success_count}"
    ...
    def test_stress_testing_simulation(self):
        ...
        ), f"Too many failures: {num_operations - success_count}"
