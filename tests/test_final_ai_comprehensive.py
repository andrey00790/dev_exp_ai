...
class FinalAIEvaluator:
    ...
    async def test_search_with_feedback_cycle(self) -> Dict[str, Any]:
        ...
        test_scenarios = [
            {"query": "OAuth 2.0 authentication", "language": "en", "expected_quality": "high"},
            {"query": "микросервисы архитектура", "language": "ru", "expected_quality": "medium"},
            {"query": "Docker deployment patterns", "language": "en", "expected_quality": "high"},
            {"query": "PostgreSQL оптимизация", "language": "ru", "expected_quality": "medium"},
            {"query": "React state management", "language": "en", "expected_quality": "high"}
        ]
        ...
        for i, scenario in enumerate(test_scenarios, 1):
            ...
            quality_score = self._evaluate_quality(search_result["data"], scenario["query"])
        ...
        avg_quality = sum(r["quality_score"] for r in all_results if r["search_success"]) / max(1, successful_searches)
        ...
        final_result = {
            ...
            "recommendation": self._generate_recommendation(avg_quality, total_feedback_sent),
            "timestamp": datetime.now().isoformat()
        }
    ...
    async def _perform_search(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self.client.post(f"{self.base_url}/api/v1/search/search", json=payload)
    ...
    async def _send_feedback(self, query: str, search_data: Dict, quality_score: float) -> Dict[str, Any]:
        """Отправляет фидбек"""
        try:
            ...
            response = await self.client.post(f"{self.base_url}/api/v1/feedback", json=feedback_payload)
    ...
    async def _trigger_retraining(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self.client.post(f"{self.base_url}/api/v1/feedback/retrain", json=retraining_payload)
            
            if response.status_code == 200:
                result = response.json()
                print(f"   ✅ Retraining job started: {result.get('job_id', 'unknown')}")
                return {"triggered": True, "job_id": result.get("job_id"), "data": result}
            else:
...
async def main():
    ...
    try:
        ...
        print(f"   🧠 Retraining: {'✅ Triggered' if results['retraining']['triggered'] else '❌ Not needed'}")
