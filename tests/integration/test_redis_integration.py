...
class TestRedisIntegration:
    ...
    def test_redis_sorted_set_operations(self, redis_client):
        ...
        # ZADD
        redis_client.zadd(
            zset_key, {"user1": 100, "user2": 200, "user3": 150, "user4": 300}
        )
    ...
    def test_redis_performance(self, redis_client):
        ...
        assert (
            write_time < 2.0
        ), f"Запись 1000 ключей заняла {write_time:.2f}s (ожидалось < 2s)"
        assert (
            read_time < 1.0
        ), f"Чтение 1000 ключей заняло {read_time:.2f}s (ожидалось < 1s)"
    ...
    def test_redis_memory_usage(self, redis_client):
        ...
        assert (
            used_memory_mb < 100
        ), f"Redis использует {used_memory_mb:.2f}MB памяти (ожидалось < 100MB)"
