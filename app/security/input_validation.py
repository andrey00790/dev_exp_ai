...
Защита от injection атак и malicious input (Pydantic v2 compatible)
"""
...
SUSPICIOUS_PATTERNS = [
    ...
    r"(OR\s+['\"]\d+['\"]\s*=\s*['\"]\d+['\"])",  # More specific OR pattern
    r"(AND\s+['\"]\d+['\"]\s*=\s*['\"]\d+['\"])",  # More specific AND pattern
    r"(\'\s+(OR|AND)\s+\'.+\')",  # Quoted OR/AND patterns
...
class UserRegistrationInput(SecureBaseModel):
    ...
    def validate_name(cls, v):
        detect_malicious_patterns(v, "name")
...
class DocumentUploadInput(SecureBaseModel):
    ...
    def validate_text_fields(cls, v):
        if v:
            detect_malicious_patterns(v, "text")
            return sanitize_html(v)
        return v
    ...
    def validate_tags(cls, value):
        if value:
            ...
            for tag in value:
                ...
                detect_malicious_patterns(tag, "tag")
...
class SearchQueryInput(SecureBaseModel):
    ...
    search_type: Optional[str] = Field("semantic", pattern=r'^(semantic|vector|hybrid|text)$', description="Search type")
    
    @field_validator('query')
    @classmethod
    def validate_query(cls, v):
        detect_malicious_patterns(v, "query")
        return sanitize_html(v)
...
class AIRequestInput(SecureBaseModel):
    ...
    prompt: str = Field(..., min_length=1, max_length=4000, description="AI prompt")
    model: Optional[str] = Field("gpt-4", pattern=r'^(gpt-3\.5-turbo|gpt-4|claude-3|llama-2)$', description="AI model")
    ...
    def validate_prompt(cls, v):
        detect_malicious_patterns(v, "prompt")
        return sanitize_html(v)
...
def sanitize_string(text: str) -> str:
    """Sanitize string - compatibility function"""
    if not isinstance(text, str):
        return text
    
    # Check for malicious patterns
    try:
        detect_malicious_patterns(text, "string")
    except SecurityValidationError:
...
def validate_email(email: str) -> str:
    """Validate email - compatibility function"""
    try:
...
def validate_input(text: str, field_name: str = "input") -> str:
    """General input validation - compatibility function"""
    if not isinstance(text, str):
        if text is None:
            return ""
        return str(text)
    ...
    # Check for malicious patterns
    try:
        detect_malicious_patterns(text, field_name)
    except SecurityValidationError:
        # Convert to ValueError for compatibility with expected message
        raise ValueError("Invalid content detected")
