...
from domain.monitoring.feedback_service import (FeedbackServiceInterface,
                                                get_feedback_service)
from models.base import BaseResponse
from models.feedback import (FeedbackAnalytics, FeedbackContext,
    ...
    try:
        ...
        if feedback.user_id:
            background_tasks.add_task(
                feedback_service.update_user_reputation, feedback.user_id, feedback
            )

        # Асинхронно проверяем, нужно ли переобучение
        background_tasks.add_task(
            feedback_service.check_retraining_needed, feedback.context
        )
...
async def get_feedback_analytics(
    ...
    try:
        ...
        analytics = await feedback_service.get_feedback_analytics(
            context=context, date_from=date_from
        )
        return analytics
    except Exception as e:
        raise HTTPException(
...
async def trigger_retraining(
    ...
    try:
        ...
        background_tasks.add_task(feedback_service.execute_retraining, job.id)
...
async def get_feedback_trends(
    ...
    try:
        ...
        trends = await feedback_service.get_feedback_trends(
            date_from=date_from, context=context
        )
        return trends
    except Exception as e:
        raise HTTPException(
...
async def delete_feedback(
    ...
    try:
        success = await feedback_service.moderate_feedback(
            feedback_id=feedback_id, is_approved=False, moderation_reason=reason
        )
        ...
        return BaseResponse(message="Обратная связь удалена")
...
async def export_feedback_data(
    ...
    try:
        ...
        export_data = await feedback_service.export_feedback_data(
            context=context, date_from=date_from, date_to=date_to, format=format
        )
