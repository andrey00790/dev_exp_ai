...
from models.feedback import (
    FeedbackRequest, UserFeedback, FeedbackStats, FeedbackAnalytics,
    FeedbackContext, RetrainingRequest, UserReputationScore,
    FeedbackType, FeedbackReason, RetrainingResponse
)
...
class FeedbackServiceInterface(ABC):
    ...
    @abstractmethod
    async def update_user_reputation(self, user_id: str, feedback: UserFeedback) -> None:
...
class MockFeedbackService(FeedbackServiceInterface):
    ...
    async def update_user_reputation(self, user_id: str, feedback: UserFeedback) -> None:
        ...
        reputation.reputation_score = min(10.0, 
            reputation.helpful_feedback_count / max(1, reputation.total_feedback_given) * 10
        )
    ...
    async def get_feedback_stats(self, target_id: str) -> Optional[FeedbackStats]:
        ...
        target_feedback = [f for f in self._feedback.values() if f.target_id == target_id]
        
        if not target_feedback:
            return None
        
        likes = sum(1 for f in target_feedback if f.feedback_type == FeedbackType.LIKE)
        dislikes = sum(1 for f in target_feedback if f.feedback_type == FeedbackType.DISLIKE)
        reports = sum(1 for f in target_feedback if f.feedback_type == FeedbackType.REPORT)
    ...
    async def get_feedback_analytics(
        ...
        positive_feedback = len([f for f in feedback_list if f.feedback_type == FeedbackType.LIKE])
        ...
        for reason in reasons:
            reason_counts[reason] = reason_counts.get(reason, 0) + 1
        top_issues = sorted(reason_counts.keys(), key=lambda x: reason_counts[x], reverse=True)[:5]
