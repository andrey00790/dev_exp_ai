...
class UsageMetricRequest(MetricRecordRequest):
    ...
    feature: str = Field(..., description="Feature being used (search, generation, etc.)")
...
class PerformanceMetricRequest(MetricRecordRequest):
    ...
    cpu_usage_percent: Optional[float] = Field(None, ge=0, le=100, description="CPU usage %")
...
class UserBehaviorRequest(MetricRecordRequest):
    ...
    event_type: str = Field(..., description="Type of event (click, view, search, etc.)")
...
class DashboardRequest(BaseModel):
    ...
    time_range: DashboardTimeRange = Field(DashboardTimeRange.LAST_7_DAYS, description="Time range")
    ...
    aggregation: AggregationPeriod = Field(AggregationPeriod.DAILY, description="Aggregation period")
...
def parse_time_range(time_range: DashboardTimeRange, start_date: Optional[datetime], end_date: Optional[datetime]) -> tuple[datetime, datetime]:
...
async def get_usage_dashboard(
    ...
    try:
        start_date, end_date = parse_time_range(request.time_range, request.start_date, request.end_date)
...
async def get_aggregated_metrics(
    ...
    try:
        ...
        if not current_user.is_admin and metric_type in [MetricType.COST, MetricType.PERFORMANCE]:
            raise HTTPException(status_code=403, detail="Admin access required for this metric type")
