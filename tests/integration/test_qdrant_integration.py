...
class QdrantClient:
    ...
    def delete_collection(self, collection_name: str) -> bool:
        try:
            response = self.session.delete(
                f"{self.url}/collections/{collection_name}", timeout=self.timeout
            )
    ...
    def delete_points(
        self, collection_name: str, point_ids: List[Union[str, int]]
    ) -> bool:
        try:
...
class TestQdrantIntegration:
        ...
        points = [
            {
                ...
                "payload": {"title": "Test Doc 1", "user_id": 1},
            },
            {
                "id": 2,
                "vector": generate_random_vector(),
                "payload": {"title": "Test Doc 2", "user_id": 1},
            },
        ]
    ...
    def test_vector_search(self, qdrant_client, test_collection):
        ...
        results = qdrant_client.search_points(
            test_collection, [0.1, 0.2, 0.3, 0.4], limit=3
        )
        ...
        points = [
            {
                "id": 201,
                "vector": generate_random_vector(),
                "payload": {"user_id": 1, "category": "work"},
            },
            {
                "id": 202,
                "vector": generate_random_vector(),
                "payload": {"user_id": 1, "category": "personal"},
            },
            {
                "id": 203,
                "vector": generate_random_vector(),
                "payload": {"user_id": 2, "category": "work"},
            },
        ]
        assert qdrant_client.upsert_points(test_collection, points) is True
        time.sleep(1)
    ...
    def test_update_points(self, qdrant_client, test_collection):
        original_point = {
            "id": 301,
            "vector": generate_random_vector(),
            "payload": {"title": "Original", "version": 1},
        }
        ...
        points = [
            {
                "id": 401,
                "vector": generate_random_vector(),
                "payload": {"title": "Delete 1"},
            },
            {
                "id": 402,
                "vector": generate_random_vector(),
                "payload": {"title": "Delete 2"},
            },
            {
                "id": 403,
                "vector": generate_random_vector(),
    ...
    def test_large_batch_operations(self, qdrant_client, test_collection):
        ...
        results = qdrant_client.search_points(
            test_collection, generate_random_vector(), limit=10
        )
    ...
    def test_collection_statistics(self, qdrant_client, test_collection):
        points = [
            {"id": 600 + i, "vector": generate_random_vector(), "payload": {"index": i}}
            for i in range(10)
        ]
        assert qdrant_client.upsert_points(test_collection, points) is True
        time.sleep(1)
