...
from models.feedback import FeedbackType, FeedbackContext, UserFeedback
...
class LearningPipelineService:
    ...
    async def process_rfc_completion(
        ...
        learning_data = await self._prepare_learning_data(rfc, session, auto_quality_score)
    ...
    async def collect_user_feedback(
        ...
        await self._update_learning_example_with_feedback(rfc_id, feedback_type, rating, comment)
...
""".strip()
        ...
        logger.info(f"Prepared learning data for RFC {rfc.id} with quality {quality.value}")
    ...
    async def _update_learning_example_with_feedback(
        ...
        learning_example.feedback_score = (learning_example.feedback_score + user_score) / 2
    ...
    async def _trigger_retraining(self) -> Dict[str, Any]:
        ...
        training_examples = [
            ex for ex in self.learning_examples.values()
        ...
        logger.info(f"✅ Model retraining completed! Used {len(training_examples)} quality examples")
