...
from services.search_interface import SearchServiceInterface
...
class SearchRequest(BaseModel):
    ...
    sources: Optional[List[str]] = Field(None, description="Источники для поиска (если не указаны - используются настройки пользователя)")
    limit: int = Field(10, description="Максимальное количество результатов", ge=1, le=100)
    ...
    search_type: str = Field("semantic", description="Тип поиска: semantic, keyword, hybrid")
...
class SearchFeedbackRequest(BaseModel):
    ...
    feedback_type: str = Field(..., description="Тип обратной связи: like, dislike, irrelevant")
...
async def _save_user_search_sources(user_id: str, sources: List[str]) -> bool:
    ...
    try:
        ...
        logger.error("Failed to save user search sources", user_id=user_id, error=str(e))
        return False
