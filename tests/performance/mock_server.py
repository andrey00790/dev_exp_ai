...
class MockAPIHandler(BaseHTTPRequestHandler):
    ...
    def _handle_health_check(self):
        ...
        if random.random() < 0.05:  # 5% error rate
            self._send_json_response({"status": "unhealthy", "error": "Service temporarily unavailable"}, 503)
        else:
    ...
    def _handle_task_submission(self, request_data):
        ...
        if random.random() < 0.15:  # 15% task rejection
            ...
            self._send_json_response({
                ...
                "status": "queued",
                "priority": priority,
                "estimated_completion": datetime.utcnow().timestamp() + random.randint(30, 300)
            })
    ...
    def _handle_optimization_endpoints(self, path, request_data):
        ...
        if 'benchmark' in path:
            ...
            self._send_json_response({
                ...
                "component": component,
                "status": "started",
                "estimated_completion": datetime.utcnow().timestamp() + random.randint(120, 600)
            })
    ...
    def _handle_optimization_history(self):
        ...
        for i in range(10):
            history.append({
                ...
                "component": random.choice(["search", "analytics", "auth", "monitoring"]),
                "type": random.choice(["cache_tuning", "query_optimization", "performance_boost"]),
