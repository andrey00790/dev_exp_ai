...
try:
    from models.data_source import (DataSourceManager, SourceMetadata,
                                    SourceType, filter_sources_by_criteria,
...
class DataSourceConfig(BaseModel):
    ...
    sync_schedule: str = Field(
        "0 2 * * *", description="Расписание синхронизации (cron)"
    )
...
class GenerationSourcesConfig(BaseModel):
    ...
    use_all_sources: bool = Field(
        True, description="Использовать все источники для генерации"
    )
    excluded_sources: List[str] = Field(
        default_factory=list, description="Исключенные источники"
    )
    ...
    try:
        ...
        for job_id, job_info in status["jobs"].items():
            ...
            documents_count = await _get_documents_count(
                job_info["source_type"], job_info["source_name"]
            )
            ...
            sources.append(
                DataSourceStatus(
                    ...
                    last_sync=(
                        datetime.fromisoformat(job_info["last_sync"])
                        if job_info["last_sync"]
                        else None
                    ),
                    next_sync=(
                        datetime.fromisoformat(job_info["next_sync"])
                        if job_info["next_sync"]
                        else None
                    ),
...
async def create_data_source(
    source_config: DataSourceConfig, current_user: dict = Depends(get_current_user)
) -> Dict[str, Any]:
    """
...
async def update_data_source(
    ...
    try:
        ...
        if success:
            return {"job_id": job_id, "message": "Data source updated successfully"}
        else:
...
async def delete_data_source(
    source_type: str, source_name: str, current_user: dict = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if success:
            return {"job_id": job_id, "message": "Data source disabled successfully"}
        else:
            raise HTTPException(status_code=404, detail="Data source not found")
...
async def update_user_search_sources(
    ...
    try:
        ...
        available_sources = await _get_available_sources()
        invalid_sources = [
            s for s in config.enabled_sources if s not in available_sources
        ]
        ...
        logger.error(
            "Failed to update user search sources", user_id=user_id, error=str(e)
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_generation_sources(
    ...
    try:
        ...
        return GenerationSourcesConfig(use_all_sources=True, excluded_sources=[])
...
async def update_generation_sources(
    config: GenerationSourcesConfig, current_user: dict = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to update generation sources"
            )
    ...
    try:
        ...
        return [
            DataSourceConfigResponse(
                ...
                metadata=(
                    SourceMetadataResponse.from_orm(source.metadata)
                    if source.metadata
                    else None
                ),
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка получения источников: {str(e)}"
        )
...
async def get_source_filters():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка получения фильтров: {str(e)}"
        )
    ...
    try:
        ...
        if category or role:
            discovered = filter_sources_by_criteria(
                discovered, category=category, role=role
            )
        ...
        for file_path in bootstrap_path.rglob("*"):
            if file_path.is_file() and file_path.suffix not in [".json"]:
                ...
                category = (
                    ...
                    if file_path.parent != bootstrap_path
                    else "root"
                )
...
async def trigger_ingestion(
    source_type: str,
    bootstrap_dir: Optional[str] = Query(
        "local/bootstrap", description="Путь к папке bootstrap для bootstrap источника"
    ),
):
    ...
    try:
        if source_type == "bootstrap":
            ...
            if result.returncode == 0:
                return {
                    "status": "success",
                    "message": "Bootstrap ingestion завершен успешно",
                }
            else:
                return {
                    "status": "error",
                    "message": f"Ошибка ingestion: {result.stderr}",
                }
        else:
            raise HTTPException(
                status_code=400, detail=f"Тип источника {source_type} не поддерживается"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка запуска ingestion: {str(e)}"
        )
