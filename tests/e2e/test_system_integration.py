    ...
    async def test_sequential_workflow_performance(self, async_client):
        ...
        for endpoint, method in operations:
            if method == "GET":
                ...
                assert response.status_code in [200, 404, 422], f"Unexpected status for {endpoint}"
...
class TestErrorHandling:
    ...
    async def test_malformed_request_handling(self, async_client):
        ...
        assert response.status_code in [307, 400, 422, 404], f"Should handle malformed JSON, got {response.status_code}"
    
    @pytest.mark.xfail(reason="Endpoints may not exist or have different auth requirements")
...
class TestSystemRecovery:
    ...
    async def test_service_degradation_handling(self, async_client):
        ...
        for endpoint in essential_endpoints:
            response = await async_client.get(endpoint)
            assert response.status_code == 200, f"Essential endpoint {endpoint} must remain available"
