    ...
    try:
        ...
        if hasattr(current_user, 'scopes') and 'admin' in current_user.scopes:
            stats["detailed_metrics"] = {
                ...
                "redis_url_configured": cache_manager.redis_url if cache_manager.redis_url else "Not configured"
            }
...
async def clear_cache_pattern(
    ...
    try:
        ...
        logger.info(f"Cache cleared by user {current_user.email}: {cleared_count} keys matching '{pattern}'")
...
async def optimize_database(
    ...
    try:
        ...
        logger.info(f"Database optimization performed by user {current_user.email}: {len(created_indexes)} indexes created")
