...
class CostController:
    ...
    def calculate_llm_cost(
        ...
        try:
            ...
            input_cost = (
                Decimal(str(prompt_tokens)) / 1000 * Decimal(str(pricing["input"]))
            )
            output_cost = (
                Decimal(str(completion_tokens)) / 1000 * Decimal(str(pricing["output"]))
            )

            total_cost = input_cost + output_cost
            logger.debug(
                f"Cost calculation: {provider}/{model} - {prompt_tokens}+{completion_tokens} tokens = ${total_cost:.6f}"
            )
    ...
    async def check_user_budget(
        self, user_id: str, estimated_cost: Decimal
    ...
    async def _get_user_budget_from_auth(
        self, user_id: str
    ...
    async def log_llm_usage(
        ...
        try:
            ...
            logger.info(
                f"LLM Usage Log: {user_id} | {provider}/{model} | {endpoint} | "
        ...
        try:
            ...
            for email, user_data in USERS_DB.items():
                if user_data.get("user_id") == user_id:
                    ...
                    logger.info(
                        f"Updated usage for {user_id}: ${current_usage:.4f} -> ${new_usage:.4f}"
                    )
                    break
...
async def cost_control_middleware(request: Request, call_next):
    ...
    try:
        ...
        # Check budget
        can_proceed, budget_info = await cost_controller.check_user_budget(
...
async def track_llm_request(
    ...
    cost = cost_controller.calculate_llm_cost(
...
async def check_budget_before_request(
    user_id: str, estimated_cost: float
) -> Tuple[bool, Dict]:
    """Check if user can afford the estimated operation cost."""
    return await cost_controller.check_user_budget(
        user_id, Decimal(str(estimated_cost))
    )
