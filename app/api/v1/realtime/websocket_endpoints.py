...
from app.websocket import (broadcast_notification, handle_websocket_connection,
...
async def get_websocket_stats():
    ...
    try:
        ...
        logger.info(
            f"✅ WebSocket stats collected: {stats_data['total_connections']} connections"
        )
...
async def _get_connection_metadata() -> dict:
    ...
    try:
        return {"last_updated": time.time(), "manager_status": "healthy"}
    except Exception as e:
...
async def send_notification(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Notification delivery failed: {str(e)}"
        )
    except Exception as e:
...
async def _send_notification_internal(
    user_id: str, notification_type: str, message: dict
) -> bool:
...
async def broadcast_message(
    ...
    try:
        ...
        logger.info(
            f"✅ Broadcast completed: {notification_type} to {broadcast_result['successful']} users"
        )
...
async def _broadcast_message_internal(
    notification_type: str, message: dict, exclude_user: Optional[str]
) -> dict:
    ...
    try:
        ...
        return {"total_recipients": 0, "successful": 0, "failed": 1}
