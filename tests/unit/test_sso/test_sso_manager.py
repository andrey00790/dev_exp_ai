...
from app.security.sso.models import (SSOProvider, SSOProviderType, SSOSession,
                                     SSOUser)
...
class TestSSOManager:
    ...
    def test_get_available_providers(
        self, sso_manager, mock_db, mock_saml_provider, mock_oauth_provider
    ):
    ...
    async def test_initiate_saml_login(self, sso_manager, mock_db, mock_saml_provider):
        ...
        # Mock SAML handler
        with patch.object(
            sso_manager.saml_handler, "initiate_login", new_callable=AsyncMock
        ) as mock_initiate:
            mock_initiate.return_value = "https://saml-redirect-url"

            # Mock _get_provider
            with patch.object(
                sso_manager, "_get_provider", return_value=mock_saml_provider
            ):
    ...
    async def test_initiate_oauth_login(
        ...
        # Mock OAuth handler
        with patch.object(
            sso_manager.oauth_handler, "initiate_login", new_callable=AsyncMock
        ) as mock_initiate:
            mock_initiate.return_value = ("https://oauth-url", "test-state")

            # Mock _get_provider
            with patch.object(
                sso_manager, "_get_provider", return_value=mock_oauth_provider
            ):
    ...
    def test_validate_provider_config_invalid_type(self, sso_manager):
        ...
        provider_data = {"name": "Test Provider", "provider_type": "invalid_type"}
    ...
    def test_delete_provider_with_active_users(
        self, sso_manager, mock_db, mock_saml_provider
    ):
    ...
    async def test_handle_logout(self, sso_manager, mock_db):
        ...
        result = await sso_manager.handle_logout(
            user_id=1, provider_id=None, db=mock_db
        )
