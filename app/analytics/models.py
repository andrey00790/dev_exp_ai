...
class UsageMetric(Base):
    ...
    )  # search, generation, etc.
    action = Column(
        String(100), nullable=False, index=True
    )  # query, generate, upload, etc.
...
class CostMetric(Base):
    ...
    )  # completion, embedding, search, etc.
...
class InsightReport(Base):
    ...
    report_type = Column(
        String(100), nullable=False, index=True
    )  # cost_optimization, performance, usage
