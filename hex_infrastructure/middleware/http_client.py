"""
Standardized HTTP client utilities for AI Assistant
...
from .async_utils import AsyncTimeouts, async_retry, with_timeout
from .exceptions import AsyncResourceError, AsyncTimeoutError
...
class StandardHttpClient:
    ...
    async def _ensure_session(self) -> aiohttp.ClientSession:
        ...
        if self._session is None or self._session.closed:
            ...
            timeout = aiohttp.ClientTimeout(
                total=self.default_timeout, connect=10.0, sock_read=30.0
            )
    ...
    async def post_json(
        ...
        response = await self.post(
            path, json_data=json_data, headers=headers, timeout=timeout
        )
...
async def http_client_context(
    ...
    client = StandardHttpClient(
        base_url=base_url, default_timeout=timeout, default_headers=headers
    )
    try:
        yield client
    finally:
...
def api_client(base_url: str, api_key: OptionalStr = None) -> StandardHttpClient:
    ...
    return http_client_factory(
        base_url=base_url, headers=headers, timeout=AsyncTimeouts.HTTP_REQUEST
    )
