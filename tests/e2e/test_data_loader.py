...
class E2EDataLoader:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt == max_attempts - 1:
                        raise Exception(
                            f"{name} not ready after {max_attempts} attempts"
                        )
                    time.sleep(30)
    ...
    def load_jira_data(self):
        ...
        jira = Jira(
            url=self.jira_url, username=self.admin_user, password=self.admin_pass
        )
        ...
        for project_data in projects_data:
            try:
                ...
                self.logger.warning(
                    f"Project {project_data['key']} might already exist: {e}"
                )
    ...
    def load_confluence_data(self):
        ...
        confluence = Confluence(
            url=self.confluence_url, username=self.admin_user, password=self.admin_pass
        )
        ...
        try:
            space = confluence.create_space(
                space_data["key"], space_data["name"], space_data["description"]
            )
        ...
        for page_data in pages_data:
            try:
                page = confluence.create_page(
                    space=space_key, title=page_data["title"], body=page_data["content"]
                )
