...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str = "anonymous"):
        ...
        logger.info(f"WebSocket connected for user {user_id}. Total connections: {len(self.user_websockets)}")
    ...
    async def disconnect(self, websocket: WebSocket):
        ...
        if websocket in self.user_websockets:
            ...
            logger.info(f"WebSocket disconnected for user {user_id}. Total connections: {len(self.user_websockets)}")
...
async def broadcast_notification(notification_type: str, data: dict, exclude_user: Optional[str] = None):
...
async def handle_websocket_message(websocket: WebSocket, user_id: str, message: Dict[str, Any]) -> None:
