...
from .models import (
    ...
    UserBehaviorMetric, AggregatedMetric, InsightReport,
...
class AnalyticsService:
        ...
        try:
            ...
            metric = await with_timeout(
                self._record_usage_metric_internal(
                    feature, action, user_id, session_id, resource,
                    duration_ms, bytes_processed, tokens_used, success,
                    error_code, error_message, metadata, **kwargs
                ),
            ...
            logger.info(f"✅ Usage metric recorded: {feature}/{action} for user {user_id}")
            return metric
            
        except AsyncTimeoutError as e:
            self.service_stats["timeout_errors"] += 1
            logger.error(f"❌ Usage metric recording timed out: {e}")
            raise HTTPException(status_code=408, detail=f"Usage metric recording timed out: {str(e)}")
        except Exception as e:
            self.service_stats["errors"] += 1
            self.db.rollback()
            logger.error(f"❌ Failed to record usage metric: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to record usage metric: {e}")
    
    async def _record_usage_metric_internal(
        self, feature: str, action: str, user_id: Optional[int], session_id: Optional[str],
        resource: Optional[str], duration_ms: Optional[float], bytes_processed: Optional[int],
        tokens_used: Optional[int], success: bool, error_code: Optional[str],
        error_message: Optional[str], metadata: Optional[Dict[str, Any]], **kwargs
    ) -> UsageMetric:
        ...
        try:
            ...
            metric = await with_timeout(
                self._record_cost_metric_internal(
                    service, operation, total_cost, user_id, organization_id,
                    model, input_tokens, output_tokens, currency, is_billable,
                    metadata, **kwargs
                ),
            ...
            logger.info(f"✅ Cost metric recorded: {service}/{operation} = ${total_cost}")
            return metric
            
        except AsyncTimeoutError as e:
            self.service_stats["timeout_errors"] += 1
            logger.error(f"❌ Cost metric recording timed out: {e}")
            raise HTTPException(status_code=408, detail=f"Cost metric recording timed out: {str(e)}")
        except Exception as e:
            self.service_stats["errors"] += 1
            self.db.rollback()
            logger.error(f"❌ Failed to record cost metric: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to record cost metric: {e}")
    
    async def _record_cost_metric_internal(
        self, service: str, operation: str, total_cost: float, user_id: Optional[int],
        organization_id: Optional[int], model: Optional[str], input_tokens: Optional[int],
        output_tokens: Optional[int], currency: str, is_billable: bool,
        metadata: Optional[Dict[str, Any]], **kwargs
    ) -> CostMetric:
    ...
    async def record_performance_metric(
        ...
        **kwargs
        ...
        try:
            ...
            asyncio.create_task(self._update_aggregations(MetricType.PERFORMANCE, metric.timestamp))
            
            logger.debug(f"Performance metric recorded: {component}/{operation} = {response_time_ms}ms")
            return metric
            
        except Exception as e:
            self.db.rollback()
            logger.error(f"Failed to record performance metric: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to record performance metric: {e}")
    ...
    async def record_user_behavior(
        ...
        **kwargs
        ...
        try:
            ...
            # Trigger async aggregation
            asyncio.create_task(self._update_aggregations(MetricType.BEHAVIOR, metric.timestamp))
            
            logger.debug(f"User behavior recorded: {event_type}/{event_name} for user {user_id}")
            return metric
            
        except Exception as e:
            self.db.rollback()
            logger.error(f"Failed to record user behavior: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to record user behavior: {e}")
    ...
    async def get_usage_dashboard(
        ...
        try:
            ...
            return await with_timeout(
                self._get_usage_dashboard_internal(start_date, end_date, user_id, aggregation),
                timeout,
            ...
            raise HTTPException(status_code=408, detail=f"Usage dashboard generation timed out: {str(e)}")
        except Exception as e:
            self.service_stats["errors"] += 1
            logger.error(f"❌ Failed to get usage dashboard: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to get usage dashboard: {e}")
    
    async def _get_usage_dashboard_internal(
        self, start_date: datetime, end_date: datetime, 
        user_id: Optional[int], aggregation: AggregationPeriod
    ) -> Dict[str, Any]:
        ...
        if isinstance(error_analytics, Exception):
            ...
            error_analytics = {"error_rate_percent": 0, "total_errors": 0, "error_breakdown": []}
    ...
    def _calculate_dashboard_timeout(self, start_date: datetime, end_date: datetime) -> float:
    ...
    async def get_cost_dashboard(
        ...
        try:
            ...
            cost_by_service = await self._get_cost_by_service(start_date, end_date, organization_id)
            
            # Get cost trends
            cost_trends = await self._get_cost_trends(start_date, end_date, organization_id, aggregation)
            
            # Get cost optimization opportunities
            optimization_opportunities = await self.insights_engine.get_cost_optimization_insights(
                start_date, end_date, organization_id
            )
            
            # Calculate total costs
            total_cost = await self._calculate_total_cost(start_date, end_date, organization_id)
            ...
            raise HTTPException(status_code=500, detail=f"Failed to get cost dashboard: {e}")
    ...
    async def get_performance_dashboard(
        ...
        try:
            ...
            slowest_endpoints = await self._get_slowest_endpoints(start_date, end_date, component)
            ...
            raise HTTPException(status_code=500, detail=f"Failed to get performance dashboard: {e}")
    ...
    async def get_service_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats_tasks = [
                ...
                self.insights_engine.get_insights_stats() if hasattr(self.insights_engine, 'get_insights_stats') else self._get_default_insights_stats()
            ]
            ...
            if total_operations > 0:
                service_stats["success_rate"] = (
                    (service_stats["metrics_recorded"] + service_stats["dashboards_generated"]) / 
                    total_operations * 100
                )
                service_stats["error_rate"] = (service_stats["errors"] / total_operations * 100)
                service_stats["timeout_rate"] = (service_stats["timeout_errors"] / total_operations * 100)
            else:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Process results
            health["service_layer"] = service_test if not isinstance(service_test, Exception) else {"status": "failed", "error": str(service_test)}
            health["aggregator"] = aggregator_test if not isinstance(aggregator_test, Exception) else {"status": "failed", "error": str(aggregator_test)}
            health["insights_engine"] = insights_test if not isinstance(insights_test, Exception) else {"status": "failed", "error": str(insights_test)}
            health["database"] = db_test if not isinstance(db_test, Exception) else {"status": "failed", "error": str(db_test)}
            ...
            all_healthy = all(
                ...
                for component in [health["service_layer"], health["aggregator"], health["insights_engine"], health["database"]]
    ...
    async def _get_top_features(
        ...
        results = query.group_by(UsageMetric.feature)\
    ...
    async def _get_error_analytics(
        ...
        error_counts = query.group_by(UsageMetric.error_code)\
                           .order_by(desc('error_count'))\
                           .limit(10)\
                           .all()
        
        total_requests = self.db.query(func.count(UsageMetric.id)).filter(
            and_(
                UsageMetric.timestamp >= start_date,
                UsageMetric.timestamp <= end_date,
            ...
            "top_errors": [
                {
                    ...
                    "percentage": round(error.error_count / total_errors * 100, 2) if total_errors > 0 else 0
                }
    ...
    async def _get_cost_by_service(
        self, start_date: datetime, end_date: datetime, organization_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """Get cost breakdown by service"""
        query = self.db.query(
            CostMetric.service,
        ...
        results = query.group_by(CostMetric.service)\
    ...
    async def _get_cost_trends(
        self, start_date: datetime, end_date: datetime,
        organization_id: Optional[int], aggregation: AggregationPeriod
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_response_time_trends(
        self, start_date: datetime, end_date: datetime,
        component: Optional[str], aggregation: AggregationPeriod
    ) -> List[Dict[str, Any]]:
        """Get response time trends"""
        return await self.aggregator.get_time_series_data(
            metric_type=MetricType.PERFORMANCE,
    ...
    async def _get_slowest_endpoints(
        ...
        results = query.group_by(PerformanceMetric.endpoint)\
