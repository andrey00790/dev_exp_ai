...
class RFCTemplateService(TemplateServiceInterface):
    ...
    async def get_template_variables(
        ...
        task_context = await self._get_task_context(
            session.task_type, session.initial_request
        )
        ...
        content = await self._generate_content_sections(
            session, answers_dict, task_context
        )
    ...
    async def _get_task_context(
        self, task_type: TaskType, initial_request: str
    ) -> Dict[str, str]:
    ...
    async def _generate_title(
        self, session: GenerationSession, answers: Dict[str, Any]
    ) -> str:
    ...
    async def _generate_content_sections(
        ...
        try:
            ...
            template_vars = await self._create_base_template_vars(
                session, answers, task_context
            )

            # Затем улучшаем их с помощью LLM
            enhanced_content = await llm_service.generate_enhanced_rfc_content(
...
""".strip(),
    ...
    def _extract_business_goals(self, answers: Dict[str, Any]) -> str:
        ...
        business_goal = answers.get(
            "q1", "Улучшение пользовательского опыта и эффективности системы"
        )
